...
Show the current status of this Cadre project: the input provided with this command (the text typed after the command name)

## 0. Mode

Pick the flow based on `the input provided with this command (the text typed after the command name)` (modes are additive; bare `/cadre-status`
...
**Run only when `the input provided with this command (the text typed after the command name)` contains `--repos` AND `cadre/repos.json` exists
