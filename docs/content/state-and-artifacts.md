...
immutable artifact format bundle. The current source creates v5 projects with schema-2 state; published v1–v4
projects remain readable and require approved refresh before delivery. Active
resources use `cadre://templates/v5/...` URIs and include SHA-256 hashes.
...
V5 operation receipts and recovery journals are generated by semantic mutations. The older operation templates remain available for legacy recovery; do not add their follow-up SHA commits to a new receipt-based operation. Active dependency context records its approval in `state.commits.dependencyContext`; archive rebases moved source paths under the batch approval while preserving archived learning.
