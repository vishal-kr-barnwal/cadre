...
export function atomicSkillMutation(root: string, sourceId: string, targetId: string | null, files: Map<string, string>): { written: string[]; removed: string[]; rollback: () => void; finish: () => void } {
  ...
  return {
    written: target ? Array.from(files.keys()).map((file) => `cadre/skills/${targetId}/${file}`).sort() : [],
    removed: !target ? [`cadre/skills/${sourceId}`] : sourceId !== targetId ? [`cadre/skills/${sourceId}`] : [],
    rollback,
