...
const MANUAL_VERIFICATION_TASK_TYPE = "user_manual_verification";
const TRACKS_INDEX_SCHEMA = "cadre.tracks_index.v1";
...
interface ArtifactRenderResult extends JsonObject {
  ok: boolean;
  artifact_id: string;
...
function writeJsonEnsured(file: string, value: JsonObject): void {
  ensureParent(file);
...
function generatedMarker(source: string, schema: string, body: string): string {
  return `<!-- cadre:generated from="${source}" schema="${schema}" hash="${textHash(body).slice(0, 16)}" -->`;
}

function withGeneratedMarker(source: string, schema: string, body: string): string {
...
function splitMarkdownSections(text: string): { title: string; body: string; sections: JsonObject[] } {
...
function markdownDocJson(kind: string, markdown: string, extras: JsonObject = {}): JsonObject {
...
function renderMarkdownDoc(value: JsonObject, fallbackTitle: string): string {
...
function markerForPlanStatus(status: unknown): string {
...
function trackSpecJsonPath(track: CadreTrack): string {
  return track.spec_json_path || path.join(track.dir, "spec.json");
}

function trackLearningsJsonlPath(track: CadreTrack): string {
  return track.learnings_jsonl_path || path.join(track.dir, "learnings.jsonl");
}

function trackHandoffJsonPath(track: CadreTrack): string {
...
function manualVerificationScope(value: unknown): string | null {
...
function isManualVerificationTaskObject(value: unknown, scope?: "phase" | "track"): boolean {
  const task = asJsonObject(value);
  const annotations = asJsonObject(task.annotations);
...
function manualVerificationCheck(id: string, heading: string, body: string, source: string): JsonObject {
  return { id, heading, body, source };
}

function phaseManualVerificationChecks(phase: JsonObject, tasks: JsonObject[]): JsonObject[] {
...
function trackManualVerificationChecks(specJson: JsonObject | null | undefined): JsonObject[] {
...
function normalizePlanManualVerification(plan: JsonObject, specJson?: JsonObject | null): JsonObject {
...
function renderPlanMarkdown(raw: JsonObject): string {
...
function normalizedSpecHeading(heading: string): string {
  return heading
    .toLowerCase()
    .replace(/&/g, " and ")
    .replace(/[^a-z0-9]+/g, " ")
...
function specSectionKind(heading: string): "description" | "functional_requirements" | "non_functional_requirements" | "acceptance_criteria" | "out_of_scope" | null {
...
function cleanSpecItem(value: JsonObject): JsonObject | null {
...
function parseSpecListItem(line: string): JsonObject {
...
function parseSpecListItems(body: string, fallbackHeading: string): JsonObject[] {
...
function specItemsFromRaw(value: unknown): JsonObject[] {
...
function specJsonFromText(trackId: string, text: string): JsonObject {
...
function normalizedSpecFromRaw(raw: JsonObject): JsonObject {
...
function appendSpecItemSection(parts: string[], heading: string, items: JsonObject[]): void {
...
function renderSpecMarkdown(raw: JsonObject): string {
...
function trackIndexPayload(root: string, tracks = listTracks(root)): JsonObject {
...
function redactRuntimeHelperPaths(text: string): string {
  return text
    .replace(/[^\s"'`]*cadre-lsp-(?:setup|review|daemon)\.js/g, "<cadre-lsp-helper>")
    .replace(/[^\s"'`]*cadre-server\.js/g, "<cadre-mcp-server>");
}

function summarizeRuntimeCommandResult(result: CommandResult): JsonObject {
  return {
    ok: result.ok,
    status: result.status,
    signal: result.signal || null,
...
function humanReviewConfirmed(args: RuntimeArgs = {}): boolean {
...
function plainReviewFile(relativePath: string, title: string, source: string, text: string): ReviewFile {
  return {
    path: relativePath,
    title,
    kind: "text",
...
function setupLspWriteRequested(args: RuntimeArgs = {}): boolean {
  const rawArgs = args as UnknownRecord;
...
function packetReviewArtifact(title: string, source: string, fields: JsonObject = {}): JsonObject {
  return {
      title,
      kind: "packet",
      source,
...
function releaseArtifactPlan(root: string, args: RuntimeArgs = {}): ReleaseArtifactPlan {
...
function releaseReviewFiles(root: string, plan: ReleaseArtifactPlan): ReviewFile[] {
  return [
    textReviewFile(
...
function artifactSchema(artifact: unknown): JsonObject {
...
function artifactDefinitions(root: string, args: RuntimeArgs = {}): ArtifactDefinition[] {
...
function artifactMatches(def: ArtifactDefinition, args: RuntimeArgs = {}): boolean {
...
function artifactCatalog(root: string, args: RuntimeArgs = {}): CoreResult {
...
function renderJsonCodeblock(title: string, value: JsonObject): string {
...
function renderJsonlMarkdown(title: string, entries: JsonObject[]): string {
  if (entries.length === 1) {
...
function releaseMarkdownFromMetadata(metadata: JsonObject): string {
...
function artifactRender(root: string, args: RuntimeArgs = {}): CoreResult {
  const artifact = asOptionalString(args.artifact || args.id);
...
function artifactValidate(root: string, args: RuntimeArgs = {}): CoreResult {
...
function artifactDiff(root: string, args: RuntimeArgs = {}): CoreResult {
  const artifacts = artifactDefinitions(root, args).filter((def) => artifactMatches(def, args));
...
function manualVerificationChecksFromInput(value: unknown): JsonObject[] {
  if (Array.isArray(value)) {
...
function manualVerificationResultObject(value: unknown): JsonObject {
  if (typeof value === "string" && value.trim()) return { summary: value.trim() };
  return asJsonObject(value);
}

function manualVerificationInputResult(args: RuntimeArgs): JsonObject {
  return manualVerificationResultObject(args.manualVerificationResult || args.manual_verification_result);
}

function manualVerificationInputSummary(args: RuntimeArgs, result: JsonObject): string {
...
function manualVerificationInputChecks(args: RuntimeArgs, result: JsonObject): JsonObject[] {
  return manualVerificationChecksFromInput(args.manualVerificationChecks || args.manual_verification_checks || result.checks);
}

function manualVerificationCommand(args: RuntimeArgs): string | null {
  return asOptionalString(args.manualVerificationCommand || args.manual_verification_command) || null;
}

function commandResultEvidence(result: CommandResult): JsonObject {
  return {
    ok: result.ok,
    status: result.status,
    signal: result.signal || null,
...
function prepareManualVerificationCompletion(
  root: string,
  track: CadreTrack,
  task: PlanTask,
  args: RuntimeArgs,
