...
- Commit only after main reports either explicit human approval in `governed` or approval-mode authorization in `phase`/`track`/`autonomous`. Commit only that task, return its distinct SHA, and wait for confirmation that main recorded the checkpoint before starting the next task.
