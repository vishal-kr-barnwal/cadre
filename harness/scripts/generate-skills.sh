#!/usr/bin/env bash
#
# generate-skills.sh — Build Claude, Codex, Copilot, and Antigravity plugin
# fixtures from the master Cadre skill.
#
...
PLUGIN_NAME="cadre"
PLUGIN_VERSION="$(node -e 'const version = require("./package.json").version; if (typeof version !== "string" || version.length === 0) process.exit(1); process.stdout.write(version)')"
