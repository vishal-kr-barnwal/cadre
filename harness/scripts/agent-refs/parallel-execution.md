...
Before using the manual mechanics below, call MCP `cadre_parallel`:

1. `action: "plan"` or `action: "next_wave"` for the ready worker set.
2. `action: "setup_workers"` for dry-run worktree commands; add `execute:true`
   only after reviewing the plan.
3. `action: "record_finish"` with `execute:true` as each worker completes.
4. `action: "merge_back"` and `action: "cleanup"` for dry-run merge/cleanup
   plans; add `execute:true` only when ready to mutate git state.

Use the manual sections below only when a packet reports a repair/fallback need
or when you need the platform-specific worker dispatch prompt.
