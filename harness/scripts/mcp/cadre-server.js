...
function setupCiTemplates(root, provider, args = {}) {
  ...
  import_node_fs17.default.mkdirSync(import_node_path31.default.dirname(target), { recursive: true });
  import_node_fs17.default.writeFileSync(target, sourceText);
...
function workflowArchive(root, args = {}) {
  ...
  for (const track of tracks) {
    ...
    import_node_fs20.default.renameSync(track.dir, target);
...
function trackPacket(deps, args) {
  ...
  if (action === "parse_plan") {
    ...
    return envelope(deps.core.parsePlanFile(import_node_path56.default.resolve(root, track.plan_json_path)));
  }
...
var JobManager = class {
  ...
  jobDir(root) {
    return import_node_path57.default.join(root, "cadre", "jobs");
  }
  jobPath(root, id) {
    return import_node_path57.default.join(this.jobDir(root), `${id}.json`);
  }
  serialize(job) {
    return {
      id: job.id,
      ...
      signal: job.signal,
      artifact_path: job.artifact_path || import_node_path57.default.relative(job.root, this.jobPath(job.root, job.id))
    };
  }
  persist(job) {
    try {
      import_node_fs30.default.mkdirSync(this.jobDir(job.root), { recursive: true });
      const artifactPath = this.jobPath(job.root, job.id);
      job.artifact_path = import_node_path57.default.relative(job.root, artifactPath);
      import_node_fs30.default.writeFileSync(artifactPath, `${JSON.stringify(this.serialize(job), null, 2)}
`);
    } catch {
    }
  }
  loadPersisted(root, id) {
    ...
    try {
      const parsed = asJsonObject(JSON.parse(import_node_fs30.default.readFileSync(file, "utf8")));
      return {
        ...parsed,
        persisted: true,
  ...
  summaryFromState(job, persisted = false) {
    ...
    return {
      ...
      artifact_path: typeof job.artifact_path === "string" ? job.artifact_path : root && id ? import_node_path57.default.relative(root, this.jobPath(root, id)) : null,
      persisted,
  ...
  persistedJobIds(root) {
    try {
      return import_node_fs30.default.readdirSync(this.jobDir(root)).filter((name) => name.endsWith(".json")).map((name) => name.slice(0, -5));
    } catch {
      return [];
    }
  }
...
function isDirectory(file) {
  try {
    return import_node_fs31.default.statSync(file).isDirectory();
  } catch {
    return false;
  }
}
function hasCadreDirectory(dir) {
  return isDirectory(import_node_path58.default.join(dir, "cadre"));
}
function isCadreStateDirectory(dir) {
  return [
    ...
    "config.json",
    "repos.json"
  ].some((name) => import_node_fs31.default.existsSync(import_node_path58.default.join(dir, name))) || isDirectory(import_node_path58.default.join(dir, "tracks"));
}
function hasCadreProjectState(dir) {
  return hasCadreDirectory(dir) && isCadreStateDirectory(import_node_path58.default.join(dir, "cadre"));
}
function normalizePathCandidate(value) {
  ...
  candidate = import_node_path58.default.resolve(candidate);
  try {
    if (import_node_fs31.default.existsSync(candidate) && !import_node_fs31.default.statSync(candidate).isDirectory()) candidate = import_node_path58.default.dirname(candidate);
  } catch {
    return null;
  }
  return candidate;
...
function findCadreRoot(start) {
  ...
  while (true) {
    ...
    if (import_node_path58.default.basename(dir) === "cadre" && isCadreStateDirectory(dir)) return import_node_path58.default.dirname(dir);
    const parent = import_node_path58.default.dirname(dir);
