...
function jsonFiles(dir) {
  try {
    return import_node_fs10.default.readdirSync(dir).filter((file) => file.endsWith(".json")).map((file) => import_node_path18.default.join(dir, file)).filter(isFile).sort();
  } catch {
    return [];
  }
}
...
function setupCiTemplates(root, provider, args = {}) {
  ...
  import_node_fs15.default.mkdirSync(import_node_path29.default.dirname(target), { recursive: true });
...
function workflowHandoff(root, args = {}) {
  ...
  if (args.execute === true) {
    ...
    import_node_fs16.default.writeFileSync(handoffPath, withGeneratedMarker(import_node_path35.default.relative(root, handoffJsonPath), "cadre.handoff.v1", renderMarkdownDoc(handoffJson, `Handoff: ${trackId}`)));
  }
  return {
    ...base,
    ok: true,
...
function workflowNewTrack(root, args = {}) {
  ...
  import_node_fs17.default.writeFileSync(import_node_path36.default.join(dir, "spec.md"), withGeneratedMarker(`cadre/tracks/${safeName(trackId)}/spec.json`, "cadre.spec.v1", renderSpecMarkdown(specJson)));
  import_node_fs17.default.writeFileSync(import_node_path36.default.join(dir, "plan.md"), withGeneratedMarker(`cadre/tracks/${safeName(trackId)}/plan.json`, "cadre.plan.v1", renderPlanMarkdown(planJson)));
  import_node_fs17.default.writeFileSync(import_node_path36.default.join(dir, "learnings.jsonl"), `${JSON.stringify(learningsEntry)}
`);
  import_node_fs17.default.writeFileSync(import_node_path36.default.join(dir, "learnings.md"), withGeneratedMarker(`cadre/tracks/${safeName(trackId)}/learnings.jsonl`, "cadre.learnings.v1", trackLearningsText(String(trackId))));
...
function workflowRevise(root, args = {}) {
  ...
  const writeResult = withTrackLock(root, track.track_id, () => {
    ...
    if (revisedSpec) {
      ...
      import_node_fs19.default.writeFileSync(track.spec_path, withGeneratedMarker(import_node_path38.default.relative(root, trackSpecJsonPath(track)), "cadre.spec.v1", renderSpecMarkdown(revisedSpec)));
    ...
    if (revisedPlan) {
      ...
      import_node_fs19.default.writeFileSync(track.plan_path, withGeneratedMarker(import_node_path38.default.relative(root, trackPlanJsonPath(track)), "cadre.plan.v1", renderPlanMarkdown(revisedPlan)));
...
function workflowSetup(root, args = {}) {
  ...
  const writeText = (relativePath, text) => {
    ...
    import_node_fs20.default.mkdirSync(import_node_path39.default.dirname(file), { recursive: true });
  ...
  import_node_fs20.default.mkdirSync(import_node_path39.default.join(cadreDir, "tracks"), { recursive: true });
  import_node_fs20.default.mkdirSync(import_node_path39.default.join(cadreDir, "archive"), { recursive: true });
  writeProjectDoc(
    "product.md",
    "product",
...
function changedEntries(root, base, head) {
  return runGit(root, ["diff", "--name-status", "--find-renames", `${base}...${head}`]).split(/\r?\n/).map((line) => line.trim()).filter(Boolean).map((line) => {
    ...
    return {
      ...
      path: file || "",
      oldPath,
      exists: file ? import_node_fs22.default.existsSync(import_node_path43.default.join(root, file)) : false
    };
  ...
  for (const file of files || []) {
    ...
    while (dir.startsWith(root)) {
      for (const name of manifestNames) {
        ...
        if (import_node_fs23.default.existsSync(candidate)) manifests.add(normalizeRel(import_node_path44.default.relative(root, candidate)));
      }
      if (dir === root) break;
    ...
    for (const candidate of candidates) {
      if (import_node_fs23.default.existsSync(import_node_path44.default.join(root, candidate))) tests.add(normalizeRel(candidate));
    }
  }
  return {
...
async function runReview(options = {}) {
  ...
  for (const server of servers) {
    ...
    try {
      ...
      for (const candidate of allCandidates.values()) {
        if (candidate.changeType !== "removed") continue;
        if (candidate.changedFile && import_node_fs24.default.existsSync(import_node_path45.default.join(root, candidate.changedFile))) continue;
        const refs = scanTextReferences(root, candidate.name, changedSet, server);
        if (refs.length > 0) {
          findings.push(externalReferenceFinding(server, candidate, refs, "text"));
        }
...
function saveConfig(configPath, config) {
  import_node_fs25.default.mkdirSync(import_node_path48.default.dirname(configPath), { recursive: true });
...
function isCadreStateDirectory(dir) {
  return [
    ...
    "config.json",
    "repos.json"
  ].some((name) => import_node_fs28.default.existsSync(import_node_path54.default.join(dir, name))) || isDirectory(import_node_path54.default.join(dir, "tracks"));
}
...
function normalizePathCandidate(value) {
  ...
  try {
    if (import_node_fs28.default.existsSync(candidate) && !import_node_fs28.default.statSync(candidate).isDirectory()) candidate = import_node_path54.default.dirname(candidate);
  } catch {
    return null;
  }
  return candidate;
