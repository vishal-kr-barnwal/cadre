...
function controlPlaneSyncSafety(root2, mode2, remote, branch) {
...
function isIgnoredFile(root2, file) {
...
function scanWorkspaceFiles(root2) {
  const counts = /* @__PURE__ */ new Map();
  const samples = /* @__PURE__ */ new Map();
  const filenameCounts = /* @__PURE__ */ new Map();
  const filenameSamples = /* @__PURE__ */ new Map();
  for (const rel of listWorkspaceFiles(root2)) {
    const full = import_node_path7.default.join(root2, rel);
...
function fileExists2(file) {
  try {
    return import_node_fs6.default.statSync(file).isFile();
  } catch {
    return false;
  }
}
...
function currentMcpServerPath(root2) {
  return mcpServerPathCandidates(root2).find(fileExists2) || null;
}
function mcpRuntimeRoot(serverPath) {
  return import_node_path10.default.resolve(import_node_path10.default.dirname(serverPath), "..", "..");
}
...
function regenIndex(root2, options = {}) {
  ...
  import_node_fs7.default.mkdirSync(import_node_path11.default.dirname(tracksFile), { recursive: true });
...
function readJsonlObjects(file, limit = 200) {
  ...
  const lines = import_node_fs9.default.readFileSync(file, "utf8").split(/\r?\n/).filter(Boolean);
...
function countJsonlLines(file) {
  ...
  return import_node_fs9.default.readFileSync(file, "utf8").split(/\r?\n/).filter(Boolean).length;
}
function ensureCadreLocalIgnored(root2) {
  ...
  import_node_fs9.default.mkdirSync(cadreDir, { recursive: true });
  const current = fileExists(ignorePath) ? import_node_fs9.default.readFileSync(ignorePath, "utf8") : "";
  const lines = current.split(/\r?\n/).filter(Boolean);
  if (!lines.includes("/local/")) {
    lines.push("/local/");
    import_node_fs9.default.writeFileSync(ignorePath, `${lines.join("\n")}
`);
...
function traceEnabled(root2, kind, args, force = false) {
  const mode2 = asOptionalString(args.commitMode || args.commit_mode)?.toLowerCase();
  if (["off", "none", "manual", "false"].includes(mode2 || "")) return false;
...
function syncControlPlane(root2, args = {}) {
  ...
  const safety = controlPlaneSyncSafety(root2, mode2, remote, branch);
  if (!safety.ok) {
...
function isFile(file) {
  try {
    return import_node_fs12.default.statSync(file).isFile();
  } catch {
    return false;
  }
}
function isDir(dir) {
  try {
    return import_node_fs12.default.statSync(dir).isDirectory();
  } catch {
    return false;
  }
}
function readJsonFile(file) {
  try {
    return asJsonObject(JSON.parse(import_node_fs12.default.readFileSync(file, "utf8")));
  } catch {
    return null;
  }
}
...
function jsonFiles(dir) {
  try {
    return import_node_fs12.default.readdirSync(dir).filter((file) => file.endsWith(".json")).map((file) => import_node_path19.default.join(dir, file)).filter(isFile).sort();
  } catch {
    return [];
  }
}
...
function referenceDirs() {
  return walkingCandidates((dir) => [
    import_node_path19.default.join(dir, "assets", "cadre", "references"),
    import_node_path19.default.join(dir, "references"),
    import_node_path19.default.join(dir, "scripts", "agent-refs"),
    import_node_path19.default.join(dir, "skills", "cadre", "references")
  ]).filter(isDir);
}
function protocolDirs() {
  return walkingCandidates((dir) => [
    import_node_path19.default.join(dir, "assets", "cadre", "protocols"),
    import_node_path19.default.join(dir, "skills", "cadre", "protocols"),
    import_node_path19.default.join(dir, "protocols")
  ]).filter(isDir);
}
function packagedSkillContract() {
  ...
  const file = findFile(walkingCandidates((dir) => [
    import_node_path19.default.join(dir, "assets", "cadre", "skill.json"),
    import_node_path19.default.join(dir, "skills", "cadre", "skill.json"),
    import_node_path19.default.join(dir, "skill.json")
  ]));
...
function packagedAgentReferences() {
  ...
  for (const file of referenceDirs().flatMap(jsonFiles)) {
    ...
    const id = asOptionalString(reference?.id) || import_node_path19.default.basename(file, ".json");
...
function packagedAgentReference(id) {
  ...
  if (embedded) return embedded;
  const file = findFile(walkingCandidates((dir) => [
    import_node_path19.default.join(dir, "assets", "cadre", "references", `${wanted}.json`),
    import_node_path19.default.join(dir, "references", `${wanted}.json`),
    import_node_path19.default.join(dir, "scripts", "agent-refs", `${wanted}.json`),
    import_node_path19.default.join(dir, "skills", "cadre", "references", `${wanted}.json`)
  ]));
  return file ? readJsonFile(file) : null;
}
...
function packagedTemplateText(relativePath) {
  ...
  return file ? import_node_fs12.default.readFileSync(file, "utf8") : null;
}
...
function compactWorkspaceHealth(value) {
  ...
  const integrations2 = asJsonObject(health.integrations);
  return {
    ok: health.ok !== false,
    ...
    integrations: isRecord(health.integrations) ? {
      summary: integrations2.summary || null,
      provider: integrations2.provider || null
    } : null
  };
}
...
function compactSetupIntegrations(value) {
  if (!value || !isRecord(value)) return null;
  const integrations2 = asJsonObject(value);
  const optionalMcps = integrations2.optional_mcps;
  return {
    summary: integrations2.summary || null,
    provider: integrations2.provider || null,
...
function targetReviewBundle(root2, workflow, args, reviewFiles2, manifestExtras) {
  ...
  for (const file of reviewFiles2) {
    ...
    const existing = exists ? import_node_fs13.default.readFileSync(targetPath, "utf8") : "";
    ...
    if (changed) {
      import_node_fs13.default.mkdirSync(import_node_path23.default.dirname(targetPath), { recursive: true });
      import_node_fs13.default.writeFileSync(targetPath, file.content);
    }
    files.push({
      path: file.path,
      review_path: targetPath,
...
function workflowReviewBundle(root2, workflow, args, reviewFiles2, manifestExtras = {}) {
  ...
  if (!explicitDir || unsafeExplicitDir) import_node_fs14.default.rmSync(directory, { recursive: true, force: true });
  import_node_fs14.default.mkdirSync(directory, { recursive: true });
  const files = reviewFiles2.map((file) => {
    const reviewPath = import_node_path24.default.join(directory, file.path);
    import_node_fs14.default.mkdirSync(import_node_path24.default.dirname(reviewPath), { recursive: true });
    import_node_fs14.default.writeFileSync(reviewPath, file.content);
    return {
      path: file.path,
...
function installedStyleGuideIds(root2) {
  ...
  try {
    return import_node_fs14.default.readdirSync(dir).filter((file) => file.endsWith(".json") && file !== "index.json").map((file) => import_node_path24.default.basename(file, ".json")).sort();
  } catch {
    return [];
  }
}
...
function listTrackDirs(root2) {
  ...
  return import_node_fs15.default.readdirSync(tracksDir, { withFileTypes: true }).filter((entry) => entry.isDirectory()).map((entry) => import_node_path26.default.join(tracksDir, entry.name)).sort();
}
...
function readApprovalSession(sessionId) {
  try {
    return JSON.parse(import_node_fs17.default.readFileSync(approvalSessionFile(sessionId), "utf8"));
  } catch {
    return null;
  }
}
function writeApprovalSession(session) {
  import_node_fs17.default.mkdirSync(approvalSessionDir(), { recursive: true });
  import_node_fs17.default.writeFileSync(approvalSessionFile(session.session_id), `${JSON.stringify(session, null, 2)}
`);
}
...
function validateApprovedTargetReviewFiles(root2, args = {}) {
  ...
  for (const file of files) {
    ...
    try {
      const content = import_node_fs17.default.readFileSync(target2, "utf8");
...
function readJsonl(file) {
  if (!fileExists(file)) return [];
  return import_node_fs18.default.readFileSync(file, "utf8").split(/\r?\n/).map((line) => line.trim()).filter(Boolean).map((line) => {
    try {
...
function renderArtifact(root2, def, args = {}) {
  ...
  if (def.sourceFormat === "markdown") {
    ...
    else body = import_node_fs18.default.readFileSync(canonicalPath, "utf8");
  ...
  const existing = projectionPath && fileExists(projectionPath) ? import_node_fs18.default.readFileSync(projectionPath, "utf8") : "";
  return {
    ok: true,
    artifact_id: def.id,
...
function artifactValidate(root2, args = {}) {
  ...
  const results = artifacts2.map((def) => {
    ...
    if (def.sourceFormat === "markdown") return { artifact_id: def.id, ok: import_node_fs18.default.readFileSync(file, "utf8").trim().length > 0, canonical_path: def.canonical };
...
function artifactSync(root2, args = {}) {
  ...
  if (execute) {
    for (const def of defs) {
      ...
      const existing = fileExists(projectionFile) ? import_node_fs18.default.readFileSync(projectionFile, "utf8") : "";
      ...
      if (!reusedReviewFiles.has(def.projection)) import_node_fs18.default.writeFileSync(projectionFile, rendered.content);
...
function readConfig(file) {
  try {
    return asJsonObject(JSON.parse(import_node_fs19.default.readFileSync(file, "utf8")));
  } catch {
    return {};
  }
}
function writeConfig(file, value) {
  import_node_fs19.default.mkdirSync(import_node_path32.default.dirname(file), { recursive: true });
  import_node_fs19.default.writeFileSync(file, `${JSON.stringify(value, null, 2)}
`);
}
...
function setupGitattributes(root2) {
  ...
  const existing = fileExists(file) ? import_node_fs20.default.readFileSync(file, "utf8") : "";
  ...
  if (changed || !fileExists(file)) {
    import_node_fs20.default.writeFileSync(file, `${lines.join("\n")}
`);
  }
...
function setupCiTemplates(root2, provider, args = {}) {
  ...
  import_node_fs20.default.mkdirSync(import_node_path34.default.dirname(target2), { recursive: true });
  import_node_fs20.default.writeFileSync(target2, sourceText);
...
function workspaceHealth(root2, args = {}) {
  ...
  const integrations2 = integrationInventory(root2, { ...args, responseMode: mode2 });
...
function integrationStatus(root2, args, kind, label, keys, mode2) {
  const topology = loadTopology(root2);
...
function integrationInventory(root2, args = {}) {
  ...
  const optionalMcps = [
    integrationStatus(root2, args, "code_search", "Code search", ["code_search", "codeSearch", "sourcegraph", "sourcegraph_mcp", "sourcegraphMcp", "search"], mode2),
    integrationStatus(root2, args, "issue_tracker", "Issue tracker", ["issue_tracker", "issueTracker", "jira", "jira_mcp", "jiraMcp", "linear", "linear_mcp", "linearMcp"], mode2),
    integrationStatus(root2, args, "ci", "CI", ["ci", "ci_provider", "ciProvider", "ci_mcp", "ciMcp", "ci_mcp_available", "ciMcpAvailable"], mode2),
    integrationStatus(root2, args, "logging", "Logging", ["logging", "observability", "telemetry", "sentry", "sentry_mcp", "datadog", "datadog_mcp", "honeycomb", "honeycomb_mcp"], mode2),
    integrationStatus(root2, args, "knowledge_base", "Knowledge base", ["knowledge_base", "knowledgeBase", "kb", "docs", "confluence", "notion", "knowledge_base_mcp", "knowledgeBaseMcp"], mode2)
  ];
...
function recommendedProviderMode(provider) {
  const mode2 = asOptionalString(provider.provider_mode) || "local";
  return ["local", "github", "gitlab"].includes(mode2) ? mode2 : "local";
}
...
function optionalMcpRecommendations(integrations2) {
  const readiness = asJsonObject(asJsonObject(integrations2).mcp_readiness);
  ...
  const rawOptional = asJsonObject(integrations2).optional_mcps;
...
function optionalMcpPrompt(integrations2) {
  const recommendations = optionalMcpRecommendations(integrations2);
...
function workflowNewTrack(root2, args = {}) {
  ...
  const writeReviewedText = (relativePath, text2) => {
    if (reusedReviewFiles.has(relativePath)) return;
    import_node_fs21.default.writeFileSync(import_node_path36.default.join(root2, relativePath), text2);
  };
...
function listFormulas(root2) {
  ...
  const items = fileExists(dir) ? import_node_fs22.default.readdirSync(dir).filter((name) => name.endsWith(".json")).map((name) => {
...
function listWisps(root2) {
  ...
  const wisps = fileExists(dir) ? import_node_fs22.default.readdirSync(dir).filter((name) => name.endsWith(".json")).map((name) => {
    const file = import_node_path37.default.join(dir, name);
...
function burnWisp(root2, args) {
  ...
  if (existed) import_node_fs22.default.rmSync(file, { force: true });
...
function referenceRecord(skillDir, value) {
  ...
  try {
    const realSkillDir = import_node_fs23.default.realpathSync(skillDir);
    const realReference = import_node_fs23.default.realpathSync(absolutePath);
    if (!isInside(realSkillDir, realReference)) return { error: `reference escapes the skill directory: ${rawPath}` };
    const stat = import_node_fs23.default.statSync(realReference);
    if (!stat.isFile() || stat.size > PROJECT_SKILL_MAX_FILE_BYTES) return { error: `invalid or oversized reference: ${rawPath}` };
    if (import_node_fs23.default.readFileSync(realReference).subarray(0, 8192).includes(0)) return { error: `binary reference is not supported: ${rawPath}` };
...
function projectSkillIds(root2) {
  ...
  try {
    return import_node_fs23.default.readdirSync(dir, { withFileTypes: true }).filter((entry) => entry.isDirectory() && import_node_fs23.default.existsSync(import_node_path42.default.join(dir, entry.name, "skill.json"))).map((entry) => entry.name).sort();
  } catch {
    return [];
  }
}
function loadProjectSkill(root2, id, knownRepos) {
  ...
  if (!import_node_fs23.default.existsSync(file)) return { id, ok: false, errors: [`skill manifest not found: ${id}/skill.json`] };
  let raw;
  let stat;
  try {
    const realCatalog = import_node_fs23.default.realpathSync(import_node_path42.default.join(root2, "cadre", "skills"));
    const realDir = import_node_fs23.default.realpathSync(skillDir);
    const realFile = import_node_fs23.default.realpathSync(file);
    ...
    stat = import_node_fs23.default.statSync(realFile);
    raw = asJsonObject(JSON.parse(import_node_fs23.default.readFileSync(realFile, "utf8")));
  } catch (error) {
  ...
  return {
    ...
    skill: {
      ...
      projectionPath: import_node_fs23.default.existsSync(projection) ? import_node_path42.default.relative(root2, projection).split(import_node_path42.default.sep).join("/") : null,
      bytes: stat.size
    }
  };
}
function projectSkillReferenceContent(reference) {
  const content = import_node_fs23.default.readFileSync(reference.absolutePath, "utf8");
...
function workflowStatus(root2, args = {}) {
  ...
  if (mode2 === "team" || args.mine === true) return { ...summary, ok: true, status: teamBoard(root2, { ...args, mine: args.mine === true }) };
  if (mode2 === "fleet" || mode2 === "repos") return { ...summary, ok: true, status: fleetStatus(root2, args) };
  if (mode2 === "available") return { ...summary, ok: true, status: availableWork(root2) };
  if (mode2 === "collisions") return { ...summary, ok: true, status: collisionScan(root2) };
  if (mode2 === "doctor") return { ...summary, ok: true, status: doctor(root2, { hasCadreProject: true }) };
...
function workflowArchive(root2, args = {}) {
  ...
  import_node_fs24.default.mkdirSync(archiveRoot, { recursive: true });
  for (const track of tracks) {
    ...
    import_node_fs24.default.renameSync(track.dir, target2);
...
function workflowHandoff(root2, args = {}) {
  ...
  if (args.execute === true) {
    ...
    if (!reusedReviewFiles.has(import_node_path43.default.relative(root2, handoffPath))) {
      import_node_fs24.default.writeFileSync(handoffPath, withGeneratedMarker(import_node_path43.default.relative(root2, handoffJsonPath), "cadre.handoff.v1", renderMarkdownDoc(handoffJson, `Handoff: ${trackId}`, import_node_path43.default.relative(root2, handoffJsonPath))));
    }
  }
...
function workflowRefresh(root2, args = {}) {
  ...
  if (args.execute === true && refreshPatterns) {
    ...
    if (refreshed) {
      if (!reusedReviewFiles.has(import_node_path44.default.relative(root2, refreshed.jsonlPath))) import_node_fs25.default.writeFileSync(refreshed.jsonlPath, refreshed.jsonl);
      if (!reusedReviewFiles.has(import_node_path44.default.relative(root2, refreshed.projectionPath))) import_node_fs25.default.writeFileSync(refreshed.projectionPath, refreshed.projection);
      patterns = {
        ok: true,
...
function workflowRelease(root2, args = {}) {
  ...
  const reusedReviewFiles = new Set(asStringArray(reviewValidation.files));
  const traceBefore = beginTrace(root2);
  import_node_fs26.default.mkdirSync(plan.releaseDir, { recursive: true });
  if (!reusedReviewFiles.has(import_node_path45.default.relative(root2, plan.releaseMd))) {
    import_node_fs26.default.writeFileSync(plan.releaseMd, plan.notes.endsWith("\n") ? plan.notes : `${plan.notes}
`);
  }
  ...
  const writeResult = withTrackLock(root2, track.track_id, () => {
    ...
    if (revisedSpec) {
      ...
      if (!reusedReviewFiles.has(import_node_path45.default.relative(root2, track.spec_path))) {
        import_node_fs26.default.writeFileSync(track.spec_path, withGeneratedMarker(import_node_path45.default.relative(root2, trackSpecJsonPath(track)), "cadre.spec.v1", renderSpecMarkdown(revisedSpec, import_node_path45.default.relative(root2, trackSpecJsonPath(track)))));
      }
    ...
    if (revisedPlan) {
      ...
      if (!reusedReviewFiles.has(import_node_path45.default.relative(root2, track.plan_path))) {
        import_node_fs26.default.writeFileSync(track.plan_path, withGeneratedMarker(import_node_path45.default.relative(root2, trackPlanJsonPath(track)), "cadre.plan.v1", renderPlanMarkdown(revisedPlan, import_node_path45.default.relative(root2, trackPlanJsonPath(track)))));
      }
...
function workflowSetup(root2, args = {}) {
  ...
  const writeText = (relativePath, text2) => {
    ...
    import_node_fs27.default.mkdirSync(import_node_path46.default.dirname(file), { recursive: true });
    import_node_fs27.default.writeFileSync(file, text2);
  ...
  import_node_fs27.default.mkdirSync(import_node_path46.default.join(cadreDir, "tracks"), { recursive: true });
  import_node_fs27.default.mkdirSync(import_node_path46.default.join(cadreDir, "archive"), { recursive: true });
...
var PROTOCOL_VERSION = "2026-07-10";
...
function artifacts(result) {
  ...
  for (const path64 of [...asStringArray(result.written), ...asStringArray(result.release_artifacts)]) files.push({ path: path64, kind: "changed" });
  return files;
}
...
function readStdin() {
  return new Promise((resolve, reject) => {
    ...
    process.stdin.on("end", () => resolve(text2));
    process.stdin.on("error", reject);
  });
}
async function runJob(payload) {
  const root2 = payload.root || process.cwd();
  const args = asJsonObject(payload.args);
  switch (asOptionalString(payload.type)) {
    case "complete_task":
      return completeTask(root2, args);
    case "coverage":
      return testCoverage(root2, args);
    case "machine_gate":
      return reviewMachineGate(root2, args);
    case "review_assist":
      return reviewAssist(root2, args);
    case "lsp_review":
      return lspReview(root2, args);
    case "lsp_impact":
      return lspImpact(root2, args);
    case "dap_snapshot":
      return dapSnapshot(root2, args);
    default:
...
if (["cadre-job-runner.js", "cadre-job-runner.ts"].includes(import_node_path48.default.basename(process.argv[1] || ""))) {
...
function languageId(file, server) {
  const ext = import_node_path49.default.extname(file);
  const basename = import_node_path49.default.basename(file).toLowerCase();
...
var LspClient = class {
  ...
  start() {
    return new Promise((resolve, reject) => {
      ...
      this.proc = (0, import_node_child_process8.spawn)(this.server.command, this.server.args || [], {
        cwd: this.root,
        stdio: ["pipe", "pipe", "pipe"]
      });
...
${body}`);
  ...
  async initialize() {
    await this.request("initialize", {
      ...
      workspaceFolders: [{ uri: (0, import_node_url.pathToFileURL)(this.root).href, name: import_node_path50.default.basename(this.root) }]
    });
    this.notify("initialized", {});
  }
  open(file) {
    const abs = import_node_path50.default.join(this.root, file);
    const uri = (0, import_node_url.pathToFileURL)(abs).href;
    const text2 = import_node_fs28.default.readFileSync(abs, "utf8");
  ...
  async documentSymbols(file) {
    return this.request("textDocument/documentSymbol", {
      textDocument: { uri: (0, import_node_url.pathToFileURL)(import_node_path50.default.join(this.root, file)).href }
    });
  }
  ...
  diagnostics(file) {
    return this.publishedDiagnostics.get((0, import_node_url.pathToFileURL)(import_node_path50.default.join(this.root, file)).href) || [];
  }
  async shutdown() {
    try {
      await withTimeout(
...
function commandAvailability(command) {
  ...
  const result = (0, import_node_child_process9.spawnSync)("sh", ["-lc", `command -v ${shellQuote(command)}`], {
    encoding: "utf8"
  });
  if (result.status === 0) {
    return {
...
function changedEntries(root2, base, head) {
  return runGit(root2, ["diff", "--name-status", "--find-renames", `${base}...${head}`]).split(/\r?\n/).map((line) => line.trim()).filter(Boolean).map((line) => {
    ...
    return {
      ...
      path: file || "",
      oldPath,
      exists: file ? import_node_fs29.default.existsSync(import_node_path51.default.join(root2, file)) : false
    };
  }).filter((entry) => Boolean(entry.path) && !isIgnoredFile(root2, entry.path));
}
function changedSymbolCandidates(root2, base, head, entry) {
  const paths = Array.from(new Set([entry.oldPath, entry.path].filter((item) => typeof item === "string" && item.length > 0)));
  const diff = runGit(root2, [
    "diff",
    "--unified=0",
    "--find-renames",
    `${base}...${head}`,
...
function serverFileMatch(file, server) {
  ...
  const ext = import_node_path52.default.extname(file);
  const basename = import_node_path52.default.basename(file).toLowerCase();
...
function scanTextReferences(root2, symbol, changedPathSet, server) {
  ...
  function visit(dir) {
    ...
    for (const entry of entries) {
      const full = import_node_path52.default.join(dir, entry.name);
      if (shouldIgnore(root2, full, entry.name)) continue;
      if (entry.isDirectory()) {
        visit(full);
      ...
      if (changedPathSet.has(import_node_path52.default.resolve(full))) continue;
      let stat;
      try {
        stat = import_node_fs30.default.statSync(full);
      } catch {
        continue;
      }
      ...
      let text2;
      try {
        text2 = import_node_fs30.default.readFileSync(full, "utf8");
      } catch {
        continue;
      }
      ...
      for (let i = 0; i < lines.length; i += 1) {
        ...
        results.push({
          file: full,
          relativeFile: normalizeRel(import_node_path52.default.relative(root2, full)),
          line: i + 1,
...
function lspRefToLocation(root2, ref) {
  ...
  try {
    ...
    return {
      file,
      relativeFile: normalizeRel(import_node_path52.default.relative(root2, file)),
...
function locationsFromResult(root2, value) {
  const items = Array.isArray(value) ? value : value ? [value] : [];
  return items.map((item) => {
    const object = asJsonObject(item);
    const ref = object.targetUri ? { uri: object.targetUri, range: object.targetSelectionRange || object.targetRange } : item;
    return lspRefToLocation(root2, ref);
  }).filter((location) => location !== null).slice(0, 20);
}
async function optionalLocations(client, kind, file, position, root2) {
  try {
    if (kind === "definition") return locationsFromResult(root2, await client.definition(file, position));
    if (kind === "typeDefinition") return locationsFromResult(root2, await client.typeDefinition(file, position));
    if (kind === "implementation") return locationsFromResult(root2, await client.implementation(file, position));
  } catch {
    return [];
  }
  return [];
...
function nearbyFileHints(root2, files) {
  const manifests = /* @__PURE__ */ new Set();
  ...
  for (const file of files || []) {
    let dir = import_node_path52.default.dirname(import_node_path52.default.join(root2, file));
    while (dir.startsWith(root2)) {
      for (const name of manifestNames) {
        const candidate = import_node_path52.default.join(dir, name);
        if (import_node_fs30.default.existsSync(candidate)) manifests.add(normalizeRel(import_node_path52.default.relative(root2, candidate)));
      }
      if (dir === root2) break;
      dir = import_node_path52.default.dirname(dir);
    }
    const parsed = import_node_path52.default.parse(file);
    const candidates = [
      import_node_path52.default.join(parsed.dir, `${parsed.name}.test${parsed.ext}`),
      import_node_path52.default.join(parsed.dir, `${parsed.name}.spec${parsed.ext}`),
      import_node_path52.default.join(parsed.dir, `${parsed.name}_test${parsed.ext}`),
      import_node_path52.default.join("test", file),
      import_node_path52.default.join("tests", file)
    ];
    for (const candidate of candidates) {
      if (import_node_fs30.default.existsSync(import_node_path52.default.join(root2, candidate))) tests.add(normalizeRel(candidate));
    }
  }
  return {
...
async function runReview(options = {}) {
  ...
  const configPath2 = import_node_path53.default.resolve(root2, args.config);
  if (!import_node_fs31.default.existsSync(configPath2)) {
    return {
      available: false,
  ...
  const config = asJsonObject(JSON.parse(import_node_fs31.default.readFileSync(configPath2, "utf8")));
  ...
  for (const entry of entries) {
    if (entry.path) changedSet.add(import_node_path53.default.resolve(root2, entry.path));
    if (entry.oldPath) changedSet.add(import_node_path53.default.resolve(root2, entry.oldPath));
  }
  for (const server of servers) {
    ...
    for (const entry of serverEntries) {
      for (const candidate of changedSymbolCandidates(root2, args.base, args.head, entry)) {
    ...
    let client = null;
    try {
      pooled = clientPool ? await clientPool.get(root2, server) : null;
      client = pooled ? pooled.client : new LspClient(root2, server);
      if (!pooled) {
      ...
      for (const file of openFiles) {
        ...
        for (const candidate of candidates) {
          ...
          if (!symbol) {
            const refs2 = scanTextReferences(root2, candidate.name, changedSet, server);
            if (refs2.length > 0) {
          ...
          const definitions = await optionalLocations(client, "definition", file, symbol.selectionRange.start, root2);
          const typeDefinitions = await optionalLocations(client, "typeDefinition", file, symbol.selectionRange.start, root2);
          const implementations = await optionalLocations(client, "implementation", file, symbol.selectionRange.start, root2);
          ...
          const externalRefs = refs.map((ref) => lspRefToLocation(root2, ref)).filter((ref) => ref !== null).filter((ref) => !changedSet.has(import_node_path53.default.resolve(ref.file))).filter((ref) => !isIgnoredFile(root2, ref.relativeFile));
      ...
      for (const candidate of allCandidates.values()) {
        if (candidate.changeType !== "removed") continue;
        if (candidate.changedFile && import_node_fs31.default.existsSync(import_node_path53.default.join(root2, candidate.changedFile))) continue;
        const refs = scanTextReferences(root2, candidate.name, changedSet, server);
        if (refs.length > 0) {
          findings.push(externalReferenceFinding(server, candidate, refs, "text"));
        }
      ...
      if (clientPool && pooled) await clientPool.drop(root2, server);
      for (const candidate of allCandidates.values()) {
        if (candidate.changeType !== "removed") continue;
        const refs = scanTextReferences(root2, candidate.name, changedSet, server);
        if (refs.length > 0) {
  ...
  return {
    ...
    fileHints: nearbyFileHints(root2, files),
...
// src/cadre-lsp-review.ts
if (["cadre-lsp-review.js", "cadre-lsp-review.ts"].includes(import_node_path54.default.basename(process.argv[1] || ""))) {
  runCli().catch((error) => {
...
var ClientPool = class {
  ...
  async get(root2, server) {
    await this.evictIdle();
    const key = this.key(root2, server);
    ...
    const client = new LspClient(root2, server);
    await client.start();
...
if (["cadre-lsp-daemon.js", "cadre-lsp-daemon.ts"].includes(import_node_path55.default.basename(process.argv[1] || ""))) {
  runLspDaemon();
}
...
function parseArgs2(argv) {
  ...
  const root2 = import_node_path56.default.resolve(args.root);
  return { ...args, root: root2, configPath: import_node_path56.default.resolve(root2, args.config) };
}
function shellQuote2(value) {
  return `'${value.replace(/'/g, "'\\''")}'`;
}
function commandAvailability2(command) {
  const result = (0, import_node_child_process11.spawnSync)("sh", ["-lc", `command -v ${shellQuote2(command)}`], {
    encoding: "utf8"
  });
  if (result.status === 0) {
    return {
...
function scanFiles(root2) {
  return scanWorkspaceFiles(root2);
}
...
function loadConfig(configPath2) {
  try {
    const parsed = JSON.parse(import_node_fs32.default.readFileSync(configPath2, "utf8"));
...
function saveConfig(configPath2, config) {
  import_node_fs32.default.mkdirSync(import_node_path56.default.dirname(configPath2), { recursive: true });
  import_node_fs32.default.writeFileSync(configPath2, `${JSON.stringify(config, null, 2)}
`);
}
function recommend(root2) {
  const scan = scanFiles(root2);
...
function workspaceFolders(root2) {
  ...
  const reposPath = import_node_path56.default.join(root2, "cadre", "repos.json");
  let repos = {};
  try {
    repos = asJsonObject(JSON.parse(import_node_fs32.default.readFileSync(reposPath, "utf8")));
  } catch {
    return folders;
  }
...
function mergeConfig(config, recommendations, root2) {
  const servers = Array.isArray(config.servers) ? [...config.servers] : [];
  const next = {
    ...config,
    servers,
    workspaceFolders: workspaceFolders(root2)
  };
...
function runCli2() {
  ...
  const result = {
    root: args.root,
    config: import_node_path56.default.relative(args.root, args.configPath),
  ...
  for (const rec of recommendations) {
    ...
    const configured2 = existingIds.has(rec.id) || existingIds.has(rec.command) ? "configured" : "not configured";
    console.log(`- ${rec.label}: ${rec.command} (${status}, ${configured2}, ${rec.files} files)`);
    if (!rec.available) console.log(`  install: ${rec.install}`);
  }
  if (written) {
    console.log(`Updated ${import_node_path56.default.relative(args.root, args.configPath)}; added: ${added.join(", ") || "none"}.`);
...
// src/cadre-lsp-setup.ts
if (["cadre-lsp-setup.js", "cadre-lsp-setup.ts"].includes(import_node_path57.default.basename(process.argv[1] || ""))) {
  try {
    runCli2();
  } catch (error) {
...
function syncedEnvelope(root2, operation, fn) {
  const syncPre = syncControlPlane(root2, { mode: "pre" });
  if (syncPre.ok === false) {
    return envelope({
      ok: false,
...
function workspaceHealth2(root2, args = {}) {
  return asJsonObject(workspaceHealth(root2, args));
}
function integrations(root2, args = {}) {
  return asJsonObject(integrationInventory(root2, args));
}
...
function agentReference(name) {
  const safeName3 = name ? safeAssetName(name) : null;
  if (!safeName3) return { ok: false, error: "name is required and must be a reference id" };
  const reference = packagedAgentReference(safeName3);
  if (!reference) return { ok: false, error: `Unknown agent reference: ${safeName3}` };
...
function resourceRead(uri, deps) {
  ...
  if (resource.base === "cadre://skill-contract") value = skillContract();
    ...
    const candidate = setupSafeProjectSkillResource && resource.root ? deps.rootResolver.rootFromCandidate(resource.root) : null;
    const root2 = candidate?.root || deps.rootResolver.requireCadreRoot(resource.root ? { root: resource.root } : {});
    if (resource.base === "cadre://team-board") value = deps.core.teamBoard(root2);
    else if (resource.base === "cadre://fleet-board") value = deps.core.fleetStatus(root2);
    else if (resource.base === "cadre://workspace-health") value = workspaceHealth2(root2, normalizedResource);
    else if (resource.base === "cadre://integrations") value = integrations(root2, normalizedResource);
    else if (resource.base === "cadre://mcp-readiness") value = deps.core.mcpReadiness(root2, normalizedResource);
    else if (resource.base === "cadre://track-context") value = deps.core.trackContext(root2, resource.trackId);
    else if (resource.base === "cadre://review-evidence") value = deps.core.reviewEvidence(root2, resource.trackId);
    else if (resource.base === "cadre://collisions") value = deps.core.collisionScan(root2);
    else if (resource.base === "cadre://repo-map") value = deps.core.repoMap(root2, resource.symbol ? { symbol: resource.symbol } : {});
    else if (resource.base === "cadre://workspace-diagnostics") value = deps.core.workspaceDiagnostics(root2);
    else if (resource.base === "cadre://lsp-status") value = { ok: true, status: deps.core.lspConfigStatus(root2), setup: deps.core.lspSetup(root2, { execute: false }) };
    else if (resource.base === "cadre://dap-status") value = { ok: true, status: deps.core.dapStatus(root2, normalizedResource), setup: deps.core.dapSetup(root2, { ...normalizedResource, execute: false }) };
    else if (resource.base === "cadre://repo-topology") value = { ok: true, root: root2, topology: deps.core.loadTopology(root2) };
    else if (resource.base === "cadre://ship-plan") value = deps.core.workflowPacket(root2, { workflow: "ship", trackId: resource.trackId || void 0 });
    else if (resource.base === "cadre://land-plan") value = deps.core.workflowPacket(root2, { workflow: "land", trackId: resource.trackId || void 0 });
    else if (resource.base === "cadre://release-plan") value = deps.core.workflowPacket(root2, { workflow: "release" });
    else if (resource.base === "cadre://my-next-actions") {
      const mine = deps.core.teamBoard(root2, { mine: true });
      const available = deps.core.availableWork(root2);
      value = {
      ...
      const board = deps.core.teamBoard(root2);
      ...
      const board = deps.core.teamBoard(root2, { mine: true });
      value = { ok: board.ok !== false, incoming_handoffs: Array.isArray(board.incoming_handoffs) ? board.incoming_handoffs : [] };
    } else if (resource.base === "cadre://parallel-state") value = deps.core.parallelWorkflow(root2, { action: "plan", trackId: resource.trackId || void 0 });
      ...
      if (!resource.trackId) value = { ok: false, error: "trackId is required" };
        ...
        integrity: deps.core.planIntegrity(root2, resource.trackId),
        review_gate: deps.core.reviewGate(root2, resource.trackId, {}),
        collisions: deps.core.collisionScan(root2)
      };
    } else if (resource.base === "cadre://test-impact") value = deps.core.testImpact(root2, {
      files: resource.files,
      base: resource.baseRef || void 0,
      head: resource.headRef || void 0
    });
    else if (resource.base === "cadre://track-plan") {
      const context = deps.core.trackContext(root2, resource.trackId);
      const track = asJsonObject(asJsonObject(context).track);
      const planPath = asOptionalString(track.plan_json_path);
      value = context.ok === false || !planPath ? context : deps.core.parsePlanFile(import_node_path58.default.resolve(root2, planPath));
    } else if (resource.base === "cadre://track-spec") {
      value = resource.trackId ? deps.core.artifactRender(root2, { artifact: `track:${resource.trackId}:spec` }) : { ok: false, error: "trackId is required" };
    } else if (resource.base === "cadre://job-result") {
      const persisted = deps.jobs.loadPersisted(root2, resource.jobId);
      ...
      const plan = asJsonObject(deps.core.workflowPacket(root2, { workflow, trackId: resource.trackId || void 0 }));
      value = {
        ok: plan.ok !== false,
        workflow,
        track_id: resource.trackId,
      ...
      value = deps.core.artifactCatalog(root2, normalizedResource);
      ...
      value = resource.artifact ? deps.core.artifactRender(root2, normalizedResource) : { ok: false, error: "artifact is required" };
    } else if (resource.base === "cadre://artifact-sync-plan") {
      value = deps.core.artifactSync(root2, { ...normalizedResource, execute: false });
    } else if (resource.base === "cadre://styleguide-selection") {
      value = {
        ok: true,
        track_id: resource.trackId,
        files: resource.files,
        catalog: deps.core.artifactCatalog(root2, { ...normalizedResource, scope: "styleguides" })
      };
    } else if (resource.base === "cadre://project-skills") {
      value = deps.core.projectSkillSelection(root2, resource.workflow || "", normalizedResource);
    } else if (resource.base === "cadre://project-skill") {
      const detail = resource.id ? deps.core.projectSkillDetail(root2, resource.id) : null;
      ...
      value = resource.reference && detail ? selectedReference ? { ok: true, skill_id: resource.id, reference: selectedReference } : { ok: false, skill_id: resource.id, error: `Unknown project-skill reference: ${resource.reference}` } : resource.id ? detail : { ok: false, error: "id is required" };
...
function repoReviewTargets(deps, root2, args) {
  const trackId = asOptionalString(args.trackId) || asOptionalString(args.track_id);
  const selected = selectedRepos(args);
  if (!trackId) return [{ repo: ".", path: ".", cwd: root2, base: args.base || "main", head: args.head || "HEAD", source: "project-root" }];
  const context = asJsonObject(deps.core.trackContext(root2, trackId));
  const topology = asJsonObject(context.topology);
  if (topology.polyrepo !== true) {
    return [{ repo: ".", path: ".", cwd: root2, base: args.base || "main", head: args.head || "HEAD", source: "project-root" }];
  }
  const track = asJsonObject(context.track);
  const fromContext = Array.isArray(context.worktrees) ? context.worktrees.map((entry) => asJsonObject(entry)) : [];
  const topologyInfo = asJsonObject(deps.core.loadTopology(root2));
  ...
  return rawTargets.map((entry) => {
    ...
    return {
      repo,
      path: rel,
      cwd: import_node_path59.default.isAbsolute(rel) ? rel : import_node_path59.default.resolve(root2, rel),
...
function annotateRepoFindings(result, target2) {
  const findings = Array.isArray(result.findings) ? result.findings.map((finding) => asJsonObject(finding)) : [];
  const repoPath = asOptionalString(target2.path) || ".";
  const cwd = asOptionalString(target2.cwd) || "";
...
async function warmLspReview(deps, root2, args) {
  const targets = repoReviewTargets(deps, root2, args);
  const timeoutMs = Number(args.timeoutMs || 12e4);
  const maxWorkers = Math.max(1, Math.min(12, Number(args.maxWorkers || args.limit || 4)));
  const rawConfig = asOptionalString(args.config) || import_node_path59.default.join(root2, "cadre", "lsp.json");
  const config = import_node_path59.default.isAbsolute(rawConfig) ? rawConfig : import_node_path59.default.resolve(root2, rawConfig);
  if (targets.length <= 1 && targets[0]?.repo === ".") {
    return asJsonObject(await deps.lspDaemon.request(
      "review",
      { ...args, root: root2, base: args.base || "main", head: args.head || "HEAD", config },
      timeoutMs
  ...
  const repos = await mapWithConcurrency(targets, maxWorkers, async (target2) => {
    ...
    if (!import_node_fs33.default.existsSync(cwd)) {
      const result2 = {
        available: false,
        reason: `Repo working root is missing: ${cwd}`,
        findings: []
      };
      return { ...target2, result: result2, findings: [] };
    }
    const result = asJsonObject(await deps.lspDaemon.request(
      "review",
      {
        ...args,
        root: cwd,
        base: asOptionalString(target2.base) || args.base || "main",
        head: asOptionalString(target2.head) || args.head || "HEAD",
        config
      },
      timeoutMs
    ).catch((error) => ({ available: false, reason: errorMessage(error), findings: [] })));
    const findings2 = annotateRepoFindings(result, target2);
    return {
      ...target2,
      repo,
...
async function workflowPacket2(deps, args) {
  ...
  if (setupWorkflows.has(workflow)) {
    ...
    return workflowEnvelope(root3, args, deps.core.workflowPacket(root3, { ...args, workflow }));
  }
  const root2 = deps.rootResolver.requireCadreRoot(args);
  if (workflow === "debug") {
    if (args.async === true) return workflowEnvelope(root2, args, { ok: true, workflow, phase_state: "running", job: deps.jobs.start("dap_snapshot", root2, { ...args, action: "dap_snapshot" }) });
    if (args.execute === true) return workflowEnvelope(root2, args, await deps.core.dapSnapshot(root2, args));
    return workflowEnvelope(root2, args, deps.core.workflowPacket(root2, { ...args, workflow }));
  }
  if ((workflow === "review" || workflow === "revise") && args.includeLsp !== false) {
    const lspResult = await warmLspReview(deps, root2, args);
    return workflowEnvelope(root2, args, deps.core.workflowPacket(root2, { ...args, workflow, lspResult }));
  }
  return workflowEnvelope(root2, args, deps.core.workflowPacket(root2, { ...args, workflow }));
}
...
function isPackagedCadreSkillDir(root2) {
  const normalized = root2.split(import_node_path60.default.sep).join("/");
  return normalized.endsWith("/harness/skills/cadre") && import_node_fs34.default.existsSync(import_node_path60.default.join(root2, "SKILL.md"));
}
...
async function projectPacket(deps, args) {
  ...
  const setupSafeActions = /* @__PURE__ */ new Set(["tech_stack_summary", "integrations"]);
  const root2 = setupSafeActions.has(String(action)) ? setupSafeRoot(deps, args).root : deps.rootResolver.requireCadreRoot(args);
  if (action === "topology") return envelope({ ok: true, root: root2, topology: deps.core.loadTopology(root2) });
  if (action === "tech_stack_summary") return envelope(deps.core.techStackSummary(root2, args));
  if (action === "integrations") return envelope(deps.core.integrationInventory(root2, args));
  if (action === "sync_control_plane") return envelope(deps.core.syncControlPlane(root2, args));
  if (action === "polyrepo_preflight") return envelope(deps.core.polyrepoPreflight(root2));
...
function statusPacket(deps, args) {
  ...
  if (action === "live") return envelope(deps.core.liveStatus(root2));
  if (action === "team") return envelope(deps.core.teamStatus(root2));
  if (action === "mine") return envelope(deps.core.teamBoard(root2, { mine: true }));
  if (action === "available") return envelope(deps.core.availableWork(root2));
  if (action === "collisions") return envelope(deps.core.collisionScan(root2));
  if (action === "board") return envelope(deps.core.teamBoard(root2, args));
  if (action === "fleet") return envelope(deps.core.fleetStatus(root2, args));
...
function trackPacket(deps, args) {
  ...
  if (action === "context") return envelope(deps.core.trackContext(root2, args.trackId || args.track_id));
  if (action === "parse_plan") {
    ...
    return envelope(deps.core.parsePlanFile(import_node_path61.default.resolve(root2, track.plan_json_path)));
  }
...
function mutatePacket(deps, args) {
  ...
  if (action === "claim") {
    ...
    return syncedEnvelope(root2, "mutate:claim", () => deps.core.claimTrack(root2, trackId, args));
  }
  if (action === "heartbeat") return syncedEnvelope(root2, "mutate:heartbeat", () => deps.core.heartbeatTrack(root2, args));
  if (action === "set_status") {
    const trackId = args.trackId || args.track_id;
    if (!trackId || !args.status) return envelope({ ok: false, error: "trackId and status are required" });
    return syncedEnvelope(root2, "mutate:set_status", () => deps.core.setTrackStatus(root2, String(trackId), String(args.status), args));
  }
  if (action === "metadata_patch") return syncedEnvelope(root2, "mutate:metadata_patch", () => deps.core.metadataPatch(root2, args));
  if (action === "record_review") return syncedEnvelope(root2, "mutate:record_review", () => deps.core.recordReview(root2, args));
  if (action === "record_worker") return syncedEnvelope(root2, "mutate:record_worker", () => deps.core.recordParallelWorker(root2, { ...args, execute: false }));
  if (action === "record_task_result") return syncedEnvelope(root2, "mutate:record_task_result", () => deps.core.recordTaskResult(root2, args));
  if (action === "regen_index") return syncedEnvelope(root2, "mutate:regen_index", () => deps.core.regenIndex(root2));
...
function parallelPacket(deps, args) {
  const root2 = deps.rootResolver.requireCadreRoot(args);
  return envelope(deps.core.parallelWorkflow(root2, args));
}
...
function jobEnvelope(type, root2, args, deps) {
  if (!type) return envelope({ ok: false, error: "job type is required" });
  return envelope({ ok: true, job: deps.jobs.start(type, root2, args) });
}
...
async function reviewPacket(deps, args) {
  ...
  if (args.async === true) return jobEnvelope(jobTypeForPacket("cadre_review", args), root2, args, deps);
  if (action === "assist") {
    let lspResult = null;
    if (args.includeLsp !== false) {
      lspResult = await warmLspReview(deps, root2, args);
    }
    const reviewArgs = { ...args };
    if (lspResult) reviewArgs.lspResult = lspResult;
    return envelope(deps.core.reviewAssist(root2, reviewArgs));
  }
  if (action === "machine_gate") return envelope(deps.core.reviewMachineGate(root2, args));
  if (action === "gate") {
    const trackId = args.trackId || args.track_id;
    if (!trackId) return envelope({ ok: false, error: "trackId is required" });
    return envelope(deps.core.reviewGate(root2, trackId, args));
  }
  if (action === "pr_ci_status") return envelope(deps.core.prCiStatus(root2, args));
  if (action === "provider_evidence") return syncedEnvelope(root2, "review:provider_evidence", () => deps.core.providerEvidence(root2, args));
  return envelope({ ok: false, error: `Unknown cadre_review action: ${action}` });
}

// src/mcp/application/packets/intel.ts
var SETUP_SAFE_READ_ACTIONS = /* @__PURE__ */ new Set([
  "repo_map",
...
async function intelPacket(deps, args) {
  ...
  if (args.async === true) return jobEnvelope(jobTypeForPacket("cadre_intel", args), root2, args, deps);
  if (action === "repo_map") return envelope(deps.core.repoMap(root2, args));
  if (action === "lsp_setup") return envelope(deps.core.lspSetup(root2, args));
  if (action === "dap_setup") return envelope(deps.core.dapSetup(root2, args));
  if (action === "dap_status") return envelope(deps.core.dapStatus(root2, args));
  if (action === "dap_snapshot") return envelope(await deps.core.dapSnapshot(root2, args));
  if (action === "workspace_diagnostics") return envelope(deps.core.workspaceDiagnostics(root2));
  if (action === "test_impact") return envelope(deps.core.testImpact(root2, args));
  if (action === "dependency_graph") return envelope(deps.core.dependencyGraph(root2));
  if (action === "mcp_readiness") return envelope(deps.core.mcpReadiness(root2, args));
  if (action === "lsp_impact") {
    ...
    return envelope(deps.core.lspImpact(root2, impactArgs));
  }
...
function jobPacket(deps, args) {
  ...
  if (action === "start") {
    ...
    return jobEnvelope(type, root2, args.args || args, deps);
  }
  if (action === "status") {
...
function artifactPacket2(deps, args) {
  ...
  if (["import", "sync"].includes(String(action)) && args.execute === true) {
    return syncedEnvelope(root2, `artifact:${action}`, () => deps.core.artifactPacket(root2, args));
  }
  return envelope(deps.core.artifactPacket(root2, args));
}
...
function actionArgs(args, action) {
  return {
...
async function actionCall(deps, args) {
  ...
  if (packet === "artifact") return asTextJson(artifactPacket2(deps, normalized));
  if (packet === "task" && action === "complete") {
    const root2 = deps.rootResolver.requireCadreRoot(normalized);
    if (normalized.async === true) return asTextJson(envelope({ ok: true, job: deps.jobs.start("complete_task", root2, normalized) }));
    return asTextJson(syncedEnvelope(root2, "complete_task", () => deps.core.completeTask(root2, { ...normalized, execute: false })));
  }
...
function createToolCall(deps) {
  return async function toolCall(name, args = {}) {
    ...
    if (name === "cadre_workflow") return asTextJson(await workflowPacket2(deps, args.input || args.approval ? workflowArgs(args) : args));
    ...
    if (name === "cadre_complete_task") {
      const root2 = deps.rootResolver.requireCadreRoot(args);
      if (args.async === true) return asTextJson(envelope({ ok: true, job: deps.jobs.start("complete_task", root2, args) }));
      return asTextJson(syncedEnvelope(root2, "complete_task", () => deps.core.completeTask(root2, { ...args, execute: false })));
    }
...
var JobManager = class {
  ...
  jobDir(root2) {
    return import_node_path62.default.join(root2, "cadre", "jobs");
  }
  jobPath(root2, id) {
    return import_node_path62.default.join(this.jobDir(root2), `${id}.json`);
  }
  serialize(job) {
    return {
      id: job.id,
      ...
      signal: job.signal,
      artifact_path: job.artifact_path || import_node_path62.default.relative(job.root, this.jobPath(job.root, job.id))
    };
  }
  persist(job) {
    try {
      import_node_fs35.default.mkdirSync(this.jobDir(job.root), { recursive: true });
      const artifactPath = this.jobPath(job.root, job.id);
      job.artifact_path = import_node_path62.default.relative(job.root, artifactPath);
      import_node_fs35.default.writeFileSync(artifactPath, `${JSON.stringify(this.serialize(job), null, 2)}
`);
    } catch {
    }
  }
  loadPersisted(root2, id) {
    if (!id) return null;
    const file = this.jobPath(root2, id);
    try {
      const parsed = asJsonObject(JSON.parse(import_node_fs35.default.readFileSync(file, "utf8")));
      return {
        ...parsed,
        persisted: true,
  ...
  summaryFromState(job, persisted = false) {
    ...
    const root2 = typeof job.root === "string" ? job.root : "";
    ...
    return {
      ...
      artifact_path: typeof job.artifact_path === "string" ? job.artifact_path : root2 && id ? import_node_path62.default.relative(root2, this.jobPath(root2, id)) : null,
      persisted,
  ...
  persistedJobIds(root2) {
    try {
      return import_node_fs35.default.readdirSync(this.jobDir(root2)).filter((name) => name.endsWith(".json")).map((name) => name.slice(0, -5));
    } catch {
      return [];
    }
  }
  ...
  start(type, root2, args = {}) {
    this.cleanup();
    const id = `job_${import_node_crypto5.default.randomUUID()}`;
    ...
    proc.stdin.end(JSON.stringify({ type, root: root2, args }));
  ...
  list(root2 = null) {
    ...
    const persisted = root2 ? this.persistedJobIds(root2).filter((id) => !this.jobs.has(id)).map((id) => this.loadPersisted(root2, id)).filter((job) => job !== null).map((job) => this.summaryFromState(job, true)) : [];
...
var LspDaemonClient = class {
  ...
  constructor() {
    ...
    this.proc = (0, import_node_child_process13.spawn)(process.execPath, [daemon, "--cadre-lsp-daemon"], {
...
function isDirectory(file) {
  try {
    return import_node_fs36.default.statSync(file).isDirectory();
  } catch {
    return false;
  }
}
function hasCadreDirectory(dir) {
  return isDirectory(import_node_path63.default.join(dir, "cadre"));
}
function isCadreStateDirectory(dir) {
  return [
    ...
    "config.json",
    "repos.json"
  ].some((name) => import_node_fs36.default.existsSync(import_node_path63.default.join(dir, name))) || isDirectory(import_node_path63.default.join(dir, "tracks"));
}
function hasCadreProjectState(dir) {
  return hasCadreDirectory(dir) && isCadreStateDirectory(import_node_path63.default.join(dir, "cadre"));
}
function normalizePathCandidate(value) {
  ...
  candidate = import_node_path63.default.resolve(candidate);
  try {
    if (import_node_fs36.default.existsSync(candidate) && !import_node_fs36.default.statSync(candidate).isDirectory()) candidate = import_node_path63.default.dirname(candidate);
  } catch {
    return null;
  }
  return candidate;
...
function findCadreRoot(start) {
  ...
  while (true) {
    ...
    if (import_node_path63.default.basename(dir) === "cadre" && isCadreStateDirectory(dir)) return import_node_path63.default.dirname(dir);
