...
function jsonFiles(dir) {
  try {
    return import_node_fs11.default.readdirSync(dir).filter((file) => file.endsWith(".json")).map((file) => import_node_path18.default.join(dir, file)).filter(isFile).sort();
  } catch {
    return [];
  }
}
...
function referenceDirs() {
  return walkingCandidates((dir) => [
    import_node_path18.default.join(dir, "assets", "cadre", "references"),
    import_node_path18.default.join(dir, "references"),
    import_node_path18.default.join(dir, "scripts", "agent-refs"),
    import_node_path18.default.join(dir, "skills", "cadre", "references")
  ]).filter(isDir);
}
function protocolDirs() {
  return walkingCandidates((dir) => [
    import_node_path18.default.join(dir, "assets", "cadre", "protocols"),
    import_node_path18.default.join(dir, "skills", "cadre", "protocols"),
    import_node_path18.default.join(dir, "protocols")
  ]).filter(isDir);
}
function packagedSkillContract() {
  ...
  const file = findFile(walkingCandidates((dir) => [
    import_node_path18.default.join(dir, "assets", "cadre", "skill.json"),
    import_node_path18.default.join(dir, "skills", "cadre", "skill.json"),
    import_node_path18.default.join(dir, "skill.json")
  ]));
...
function packagedAgentReferences() {
  ...
  for (const file of referenceDirs().flatMap(jsonFiles)) {
    ...
    const id = asOptionalString(reference?.id) || import_node_path18.default.basename(file, ".json");
...
function packagedAgentReference(id) {
  ...
  if (embedded) return embedded;
  const file = findFile(walkingCandidates((dir) => [
    import_node_path18.default.join(dir, "assets", "cadre", "references", `${wanted}.json`),
    import_node_path18.default.join(dir, "references", `${wanted}.json`),
    import_node_path18.default.join(dir, "scripts", "agent-refs", `${wanted}.json`),
    import_node_path18.default.join(dir, "skills", "cadre", "references", `${wanted}.json`)
  ]));
  return file ? readJsonFile(file) : null;
}
...
function workflowReviewBundle(root, workflow, args, reviewFiles, manifestExtras = {}) {
  ...
  const files = reviewFiles.map((file) => {
    ...
    import_node_fs12.default.mkdirSync(import_node_path22.default.dirname(reviewPath), { recursive: true });
...
function installedStyleGuideIds(root) {
  ...
  try {
    return import_node_fs12.default.readdirSync(dir).filter((file) => file.endsWith(".json") && file !== "index.json").map((file) => import_node_path22.default.basename(file, ".json")).sort();
  } catch {
    return [];
  }
}
...
function listTrackDirs(root) {
  ...
  return import_node_fs13.default.readdirSync(tracksDir, { withFileTypes: true }).filter((entry) => entry.isDirectory()).map((entry) => import_node_path24.default.join(tracksDir, entry.name)).sort();
}
...
function readConfig(file) {
  try {
    return asJsonObject(JSON.parse(import_node_fs16.default.readFileSync(file, "utf8")));
  } catch {
    return {};
  }
}
function writeConfig(file, value) {
  import_node_fs16.default.mkdirSync(import_node_path29.default.dirname(file), { recursive: true });
  import_node_fs16.default.writeFileSync(file, `${JSON.stringify(value, null, 2)}
`);
}
...
function setupGitattributes(root) {
  ...
  const existing = fileExists(file) ? import_node_fs17.default.readFileSync(file, "utf8") : "";
  ...
  if (changed || !fileExists(file)) {
    import_node_fs17.default.writeFileSync(file, `${lines.join("\n")}
`);
  }
...
function setupCiTemplates(root, provider, args = {}) {
  ...
  import_node_fs17.default.mkdirSync(import_node_path31.default.dirname(target2), { recursive: true });
  import_node_fs17.default.writeFileSync(target2, sourceText);
...
function workflowNewTrack(root, args = {}) {
  ...
  import_node_fs18.default.writeFileSync(import_node_path33.default.join(dir, "spec.md"), withGeneratedMarker(`cadre/tracks/${safeName(trackId)}/spec.json`, "cadre.spec.v1", renderSpecMarkdown(specJson, `cadre/tracks/${safeName(trackId)}/spec.json`)));
  import_node_fs18.default.writeFileSync(import_node_path33.default.join(dir, "plan.md"), withGeneratedMarker(`cadre/tracks/${safeName(trackId)}/plan.json`, "cadre.plan.v1", renderPlanMarkdown(planJson, `cadre/tracks/${safeName(trackId)}/plan.json`)));
  import_node_fs18.default.writeFileSync(import_node_path33.default.join(dir, "learnings.jsonl"), `${JSON.stringify(learningsEntry)}
`);
  import_node_fs18.default.writeFileSync(import_node_path33.default.join(dir, "learnings.md"), withGeneratedMarker(`cadre/tracks/${safeName(trackId)}/learnings.jsonl`, "cadre.learnings.v1", trackLearningsText(String(trackId))));
  const regen = regenIndex(root);
  const event = appendCadreEvent(root, {
    kind: "track_created",
    workflow: "newtrack",
...
function listFormulas(root) {
  ...
  const items = fileExists(dir) ? import_node_fs19.default.readdirSync(dir).filter((name) => name.endsWith(".json")).map((name) => {
...
function listWisps(root) {
  ...
  const wisps = fileExists(dir) ? import_node_fs19.default.readdirSync(dir).filter((name) => name.endsWith(".json")).map((name) => {
    const file = import_node_path34.default.join(dir, name);
...
function burnWisp(root, args) {
  ...
  if (existed) import_node_fs19.default.rmSync(file, { force: true });
...
function workflowArchive(root, args = {}) {
  ...
  import_node_fs20.default.mkdirSync(archiveRoot, { recursive: true });
  for (const track of tracks) {
    ...
    import_node_fs20.default.renameSync(track.dir, target2);
...
function workflowHandoff(root, args = {}) {
  ...
  if (args.execute === true) {
    ...
    import_node_fs20.default.writeFileSync(handoffPath, withGeneratedMarker(import_node_path39.default.relative(root, handoffJsonPath), "cadre.handoff.v1", renderMarkdownDoc(handoffJson, `Handoff: ${trackId}`, import_node_path39.default.relative(root, handoffJsonPath))));
  }
...
function workflowRefresh(root, args = {}) {
  ...
  if (args.execute === true && refreshPatterns) {
    ...
    if (refreshed) {
      import_node_fs21.default.writeFileSync(refreshed.jsonlPath, refreshed.jsonl);
      import_node_fs21.default.writeFileSync(refreshed.projectionPath, refreshed.projection);
      patterns = {
        ok: true,
...
function workflowRelease(root, args = {}) {
  ...
  const traceBefore = beginTrace(root);
  import_node_fs22.default.mkdirSync(plan.releaseDir, { recursive: true });
  import_node_fs22.default.writeFileSync(plan.releaseMd, plan.notes.endsWith("\n") ? plan.notes : `${plan.notes}
`);
...
function workflowRevise(root, args = {}) {
  ...
  const writeResult = withTrackLock(root, track.track_id, () => {
    ...
    if (revisedSpec) {
      ...
      import_node_fs22.default.writeFileSync(track.spec_path, withGeneratedMarker(import_node_path41.default.relative(root, trackSpecJsonPath(track)), "cadre.spec.v1", renderSpecMarkdown(revisedSpec, import_node_path41.default.relative(root, trackSpecJsonPath(track)))));
    ...
    if (revisedPlan) {
      ...
      import_node_fs22.default.writeFileSync(track.plan_path, withGeneratedMarker(import_node_path41.default.relative(root, trackPlanJsonPath(track)), "cadre.plan.v1", renderPlanMarkdown(revisedPlan, import_node_path41.default.relative(root, trackPlanJsonPath(track)))));
...
function workflowSetup(root, args = {}) {
  ...
  const writeText = (relativePath, text) => {
    ...
    import_node_fs23.default.mkdirSync(import_node_path42.default.dirname(file), { recursive: true });
    import_node_fs23.default.writeFileSync(file, text);
  ...
  import_node_fs23.default.mkdirSync(import_node_path42.default.join(cadreDir, "tracks"), { recursive: true });
  import_node_fs23.default.mkdirSync(import_node_path42.default.join(cadreDir, "archive"), { recursive: true });
...
if (["cadre-job-runner.js", "cadre-job-runner.ts"].includes(import_node_path44.default.basename(process.argv[1] || ""))) {
...
function languageId(file, server) {
  const ext = import_node_path45.default.extname(file);
  const basename = import_node_path45.default.basename(file).toLowerCase();
...
var LspClient = class {
  ...
  start() {
    return new Promise((resolve, reject) => {
      ...
      this.proc = (0, import_node_child_process7.spawn)(this.server.command, this.server.args || [], {
        cwd: this.root,
        stdio: ["pipe", "pipe", "pipe"]
      });
...
${body}`);
  ...
  async initialize() {
    await this.request("initialize", {
      ...
      workspaceFolders: [{ uri: (0, import_node_url.pathToFileURL)(this.root).href, name: import_node_path46.default.basename(this.root) }]
    });
    this.notify("initialized", {});
  }
  open(file) {
    const abs = import_node_path46.default.join(this.root, file);
    const uri = (0, import_node_url.pathToFileURL)(abs).href;
    const text = import_node_fs24.default.readFileSync(abs, "utf8");
  ...
  async documentSymbols(file) {
    return this.request("textDocument/documentSymbol", {
      textDocument: { uri: (0, import_node_url.pathToFileURL)(import_node_path46.default.join(this.root, file)).href }
    });
  }
  ...
  diagnostics(file) {
    return this.publishedDiagnostics.get((0, import_node_url.pathToFileURL)(import_node_path46.default.join(this.root, file)).href) || [];
  }
  async shutdown() {
    try {
      await withTimeout(
...
function commandAvailability(command) {
  ...
  const result = (0, import_node_child_process8.spawnSync)("sh", ["-lc", `command -v ${shellQuote2(command)}`], {
    encoding: "utf8"
  });
  if (result.status === 0) {
    return {
...
function runGit(root, args) {
  const result = (0, import_node_child_process9.spawnSync)("git", args, { cwd: root, encoding: "utf8" });
  if (result.status !== 0) {
...
function changedEntries(root, base, head) {
  return runGit(root, ["diff", "--name-status", "--find-renames", `${base}...${head}`]).split(/\r?\n/).map((line) => line.trim()).filter(Boolean).map((line) => {
    ...
    return {
      ...
      path: file || "",
      oldPath,
      exists: file ? import_node_fs25.default.existsSync(import_node_path47.default.join(root, file)) : false
    };
...
function serverFileMatch(file, server) {
  ...
  const ext = import_node_path48.default.extname(file);
  const basename = import_node_path48.default.basename(file).toLowerCase();
...
function scanTextReferences(root, symbol, changedPathSet, server) {
  ...
  function visit(dir) {
    ...
    for (const entry of entries) {
      const full = import_node_path48.default.join(dir, entry.name);
      if (shouldIgnore(root, full, entry.name)) continue;
      if (entry.isDirectory()) {
        visit(full);
      ...
      if (changedPathSet.has(import_node_path48.default.resolve(full))) continue;
      let stat;
      try {
        stat = import_node_fs26.default.statSync(full);
      } catch {
        continue;
      }
      ...
      let text;
      try {
        text = import_node_fs26.default.readFileSync(full, "utf8");
      } catch {
        continue;
      }
      ...
      for (let i = 0; i < lines.length; i += 1) {
        ...
        results.push({
          file: full,
          relativeFile: normalizeRel(import_node_path48.default.relative(root, full)),
          line: i + 1,
...
function lspRefToLocation(root, ref) {
  ...
  try {
    ...
    return {
      file,
      relativeFile: normalizeRel(import_node_path48.default.relative(root, file)),
...
function nearbyFileHints(root, files) {
  ...
  for (const file of files || []) {
    let dir = import_node_path48.default.dirname(import_node_path48.default.join(root, file));
    while (dir.startsWith(root)) {
      for (const name of manifestNames) {
        const candidate = import_node_path48.default.join(dir, name);
        if (import_node_fs26.default.existsSync(candidate)) manifests.add(normalizeRel(import_node_path48.default.relative(root, candidate)));
      }
      if (dir === root) break;
      dir = import_node_path48.default.dirname(dir);
    }
    const parsed = import_node_path48.default.parse(file);
    const candidates = [
      import_node_path48.default.join(parsed.dir, `${parsed.name}.test${parsed.ext}`),
      import_node_path48.default.join(parsed.dir, `${parsed.name}.spec${parsed.ext}`),
      import_node_path48.default.join(parsed.dir, `${parsed.name}_test${parsed.ext}`),
      import_node_path48.default.join("test", file),
      import_node_path48.default.join("tests", file)
    ];
    for (const candidate of candidates) {
      if (import_node_fs26.default.existsSync(import_node_path48.default.join(root, candidate))) tests.add(normalizeRel(candidate));
    }
  }
  return {
...
async function runReview(options = {}) {
  ...
  const configPath2 = import_node_path49.default.resolve(root, args.config);
  if (!import_node_fs27.default.existsSync(configPath2)) {
    return {
      available: false,
  ...
  const config = asJsonObject(JSON.parse(import_node_fs27.default.readFileSync(configPath2, "utf8")));
  ...
  for (const entry of entries) {
    if (entry.path) changedSet.add(import_node_path49.default.resolve(root, entry.path));
    if (entry.oldPath) changedSet.add(import_node_path49.default.resolve(root, entry.oldPath));
  }
    ...
    try {
      ...
      for (const file of openFiles) {
        ...
        for (const candidate of candidates) {
          ...
          const externalRefs = refs.map((ref) => lspRefToLocation(root, ref)).filter((ref) => ref !== null).filter((ref) => !changedSet.has(import_node_path49.default.resolve(ref.file))).filter((ref) => !isIgnoredFile(root, ref.relativeFile));
      ...
      for (const candidate of allCandidates.values()) {
        if (candidate.changeType !== "removed") continue;
        if (candidate.changedFile && import_node_fs27.default.existsSync(import_node_path49.default.join(root, candidate.changedFile))) continue;
        const refs = scanTextReferences(root, candidate.name, changedSet, server);
        if (refs.length > 0) {
          findings.push(externalReferenceFinding(server, candidate, refs, "text"));
        }
...
// src/cadre-lsp-review.ts
if (["cadre-lsp-review.js", "cadre-lsp-review.ts"].includes(import_node_path50.default.basename(process.argv[1] || ""))) {
  runCli().catch((error) => {
...
if (["cadre-lsp-daemon.js", "cadre-lsp-daemon.ts"].includes(import_node_path51.default.basename(process.argv[1] || ""))) {
  runLspDaemon();
}
...
function parseArgs2(argv) {
  ...
  const root = import_node_path52.default.resolve(args.root);
  return { ...args, root, configPath: import_node_path52.default.resolve(root, args.config) };
}
function shellQuote3(value) {
  return `'${value.replace(/'/g, "'\\''")}'`;
}
function commandAvailability2(command) {
  const result = (0, import_node_child_process10.spawnSync)("sh", ["-lc", `command -v ${shellQuote3(command)}`], {
    encoding: "utf8"
  });
  if (result.status === 0) {
    return {
...
function loadConfig(configPath2) {
  try {
    const parsed = JSON.parse(import_node_fs28.default.readFileSync(configPath2, "utf8"));
...
function saveConfig(configPath2, config) {
  import_node_fs28.default.mkdirSync(import_node_path52.default.dirname(configPath2), { recursive: true });
  import_node_fs28.default.writeFileSync(configPath2, `${JSON.stringify(config, null, 2)}
`);
}
...
function workspaceFolders(root) {
  ...
  const reposPath = import_node_path52.default.join(root, "cadre", "repos.json");
  let repos = {};
  try {
    repos = asJsonObject(JSON.parse(import_node_fs28.default.readFileSync(reposPath, "utf8")));
  } catch {
    return folders;
  }
...
function runCli2() {
  ...
  const result = {
    root: args.root,
    config: import_node_path52.default.relative(args.root, args.configPath),
  ...
  if (written) {
    console.log(`Updated ${import_node_path52.default.relative(args.root, args.configPath)}; added: ${added.join(", ") || "none"}.`);
...
// src/cadre-lsp-setup.ts
if (["cadre-lsp-setup.js", "cadre-lsp-setup.ts"].includes(import_node_path53.default.basename(process.argv[1] || ""))) {
  try {
    runCli2();
  } catch (error) {
...
function resourceRead(uri, deps) {
  ...
  if (resource.base === "cadre://skill-contract") value = skillContract();
    ...
    if (resource.base === "cadre://team-board") value = deps.core.teamBoard(root);
      ...
      value = context.ok === false || !planPath ? context : deps.core.parsePlanFile(import_node_path54.default.resolve(root, planPath));
...
function repoReviewTargets(deps, root, args) {
  ...
  return rawTargets.map((entry) => {
    ...
    return {
      repo,
      path: rel,
      cwd: import_node_path55.default.isAbsolute(rel) ? rel : import_node_path55.default.resolve(root, rel),
...
async function warmLspReview(deps, root, args) {
  ...
  const rawConfig = asOptionalString(args.config) || import_node_path55.default.join(root, "cadre", "lsp.json");
  const config = import_node_path55.default.isAbsolute(rawConfig) ? rawConfig : import_node_path55.default.resolve(root, rawConfig);
  ...
  const repos = await mapWithConcurrency(targets, maxWorkers, async (target2) => {
    const cwd = asOptionalString(target2.cwd) || root;
    const repo = asOptionalString(target2.repo) || ".";
    if (!import_node_fs29.default.existsSync(cwd)) {
      const result2 = {
        available: false,
...
function isPackagedCadreSkillDir(root) {
  const normalized = root.split(import_node_path56.default.sep).join("/");
  return normalized.endsWith("/harness/skills/cadre") && import_node_fs30.default.existsSync(import_node_path56.default.join(root, "SKILL.md"));
}
...
async function projectPacket(deps, args) {
  ...
  const root = setupSafeActions.has(String(action)) ? setupSafeRoot(deps, args).root : deps.rootResolver.requireCadreRoot(args);
...
function trackPacket(deps, args) {
  ...
  if (action === "parse_plan") {
    ...
    return envelope(deps.core.parsePlanFile(import_node_path57.default.resolve(root, track.plan_json_path)));
  }
...
async function intelPacket(deps, args) {
  ...
  const root = usesCandidateRoot ? rootInfo ? rootInfo.root : process.cwd() : deps.rootResolver.requireCadreRoot(args);
...
var JobManager = class {
  ...
  jobDir(root) {
    return import_node_path58.default.join(root, "cadre", "jobs");
  }
  jobPath(root, id) {
    return import_node_path58.default.join(this.jobDir(root), `${id}.json`);
  }
  serialize(job) {
    return {
      id: job.id,
      ...
      signal: job.signal,
      artifact_path: job.artifact_path || import_node_path58.default.relative(job.root, this.jobPath(job.root, job.id))
    };
  }
  persist(job) {
    try {
      import_node_fs31.default.mkdirSync(this.jobDir(job.root), { recursive: true });
      const artifactPath = this.jobPath(job.root, job.id);
      job.artifact_path = import_node_path58.default.relative(job.root, artifactPath);
      import_node_fs31.default.writeFileSync(artifactPath, `${JSON.stringify(this.serialize(job), null, 2)}
`);
    } catch {
    }
  }
  loadPersisted(root, id) {
    ...
    try {
      const parsed = asJsonObject(JSON.parse(import_node_fs31.default.readFileSync(file, "utf8")));
      return {
        ...parsed,
        persisted: true,
  ...
  summaryFromState(job, persisted = false) {
    ...
    return {
      ...
      artifact_path: typeof job.artifact_path === "string" ? job.artifact_path : root && id ? import_node_path58.default.relative(root, this.jobPath(root, id)) : null,
      persisted,
  ...
  persistedJobIds(root) {
    try {
      return import_node_fs31.default.readdirSync(this.jobDir(root)).filter((name) => name.endsWith(".json")).map((name) => name.slice(0, -5));
    } catch {
      return [];
    }
  }
  ...
  start(type, root, args = {}) {
    ...
    const proc = (0, import_node_child_process11.spawn)(process.execPath, [runner, "--cadre-job-runner"], {
...
var LspDaemonClient = class {
  ...
  constructor() {
    ...
    this.proc = (0, import_node_child_process12.spawn)(process.execPath, [daemon, "--cadre-lsp-daemon"], {
...
function isDirectory(file) {
  try {
    return import_node_fs32.default.statSync(file).isDirectory();
  } catch {
    return false;
  }
}
function hasCadreDirectory(dir) {
  return isDirectory(import_node_path59.default.join(dir, "cadre"));
}
function isCadreStateDirectory(dir) {
  return [
    ...
    "config.json",
    "repos.json"
  ].some((name) => import_node_fs32.default.existsSync(import_node_path59.default.join(dir, name))) || isDirectory(import_node_path59.default.join(dir, "tracks"));
}
function hasCadreProjectState(dir) {
  return hasCadreDirectory(dir) && isCadreStateDirectory(import_node_path59.default.join(dir, "cadre"));
}
function normalizePathCandidate(value) {
  ...
  candidate = import_node_path59.default.resolve(candidate);
  try {
    if (import_node_fs32.default.existsSync(candidate) && !import_node_fs32.default.statSync(candidate).isDirectory()) candidate = import_node_path59.default.dirname(candidate);
  } catch {
    return null;
  }
  return candidate;
...
function findCadreRoot(start) {
  ...
  while (true) {
    ...
    if (import_node_path59.default.basename(dir) === "cadre" && isCadreStateDirectory(dir)) return import_node_path59.default.dirname(dir);
    const parent = import_node_path59.default.dirname(dir);
