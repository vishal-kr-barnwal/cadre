...
function setupCiTemplates(root, provider, args = {}) {
  ...
  import_node_fs16.default.mkdirSync(import_node_path29.default.dirname(target), { recursive: true });
...
function workflowNewTrack(root, args = {}) {
  ...
  const traceBefore = beginTrace(root);
  const dir = import_node_path31.default.join(root, "cadre", "tracks", safeName(trackId));
  const learningsEntry = {
    ...templateJson("learnings_seed.json", { id: "initial", kind: "learnings_seed" }),
    id: "initial",
    kind: "learnings_seed",
  ...
  writeJson(import_node_path31.default.join(dir, "metadata.json"), metadata);
  writeJson(import_node_path31.default.join(dir, "spec.json"), specJson);
  writeJson(import_node_path31.default.join(dir, "plan.json"), planJson);
  import_node_fs17.default.writeFileSync(import_node_path31.default.join(dir, "spec.md"), withGeneratedMarker(`cadre/tracks/${safeName(trackId)}/spec.json`, "cadre.spec.v1", renderSpecMarkdown(specJson)));
  import_node_fs17.default.writeFileSync(import_node_path31.default.join(dir, "plan.md"), withGeneratedMarker(`cadre/tracks/${safeName(trackId)}/plan.json`, "cadre.plan.v1", renderPlanMarkdown(planJson)));
  import_node_fs17.default.writeFileSync(import_node_path31.default.join(dir, "learnings.jsonl"), `${JSON.stringify(learningsEntry)}
`);
  import_node_fs17.default.writeFileSync(import_node_path31.default.join(dir, "learnings.md"), withGeneratedMarker(`cadre/tracks/${safeName(trackId)}/learnings.jsonl`, "cadre.learnings.v1", trackLearningsText(String(trackId))));
  const regen = regenIndex(root);
  const event = appendCadreEvent(root, {
    kind: "track_created",
    workflow: "newtrack",
  ...
  return {
    ...
    dry_run: false,
    track_id: trackId,
    metadata_path: import_node_path31.default.relative(root, import_node_path31.default.join(dir, "metadata.json")),
    regen,
    event,
    control_commit: controlCommit,
...
function formulaDir(root) {
  return import_node_path32.default.join(root, "cadre", "formulas");
}
function wispDir(root) {
  return import_node_path32.default.join(root, "cadre", "local", "wisps");
}
...
function formulaPath(root, id) {
  return import_node_path32.default.join(formulaDir(root), `${safeName(id).replace(/\.json$/i, "")}.json`);
}
function wispPath(root, id) {
  return import_node_path32.default.join(wispDir(root), `${safeName(id).replace(/\.json$/i, "")}.json`);
}
function loadFormula(root, id) {
  ...
  if (!formula) return { ok: false, id, error: `Formula not found: ${id}`, path: import_node_path32.default.relative(root, file) };
  return { ok: true, id: asOptionalString(formula.id) || id, path: import_node_path32.default.relative(root, file), formula };
}
...
function listFormulas(root) {
  ...
  const items = fileExists(dir) ? import_node_fs18.default.readdirSync(dir).filter((name) => name.endsWith(".json")).map((name) => {
    const file = import_node_path32.default.join(dir, name);
    ...
    return {
      ...
      recommended_phase: asOptionalString(formula.recommended_phase) || null,
      path: import_node_path32.default.relative(root, file)
    };
  }) : [];
...
function createWisp(root, args) {
  ...
  return { ok: true, wisp_id: id, path: import_node_path32.default.relative(root, file), wisp, event };
}
...
function updateWispStep(root, args) {
  ...
  if (!wisp) return { ok: false, error: `Wisp not found: ${id}`, path: import_node_path32.default.relative(root, file) };
  const rawArgs = args;
  ...
  return { ok: true, wisp_id: id, path: import_node_path32.default.relative(root, file), step: steps[index], wisp: next, event };
}
function squashWisp(root, args) {
  ...
  const digestPath = import_node_path32.default.join(root, "cadre", "operations", "wisp-digests.jsonl");
  ...
  const controlCommit = commitTrace(root, args, {
    ...
    before: traceBefore,
    files: [
      import_node_path32.default.relative(root, digestPath)
    ],
    note: {
      event_id: asOptionalString(asJsonObject(event.event).id) || null,
      wisp_id: id,
  ...
  return { ok: controlCommit.ok !== false, wisp_id: id, digest, path: import_node_path32.default.relative(root, digestPath), event, control_commit: controlCommit };
}
function burnWisp(root, args) {
  ...
  return { ok: true, wisp_id: id, existed, path: import_node_path32.default.relative(root, file), event };
}
...
function completionJournalPath(track) {
  return import_node_path33.default.join(track.dir, "completion_journal.json");
}
...
function patchCompletionJournal(track, key, patcher) {
  ...
  appendJsonl(import_node_path33.default.join(track.dir, "completion_journal.jsonl"), {
    key,
...
function completeTaskInner(root, args = {}) {
  ...
  const recordState = () => {
    ...
    const taskResult = recordTaskResultUnlocked(root, {
      ...
      coverage: coverage.coverage,
      repo: workingRoot.repo,
      workingRoot: import_node_path34.default.relative(root, workingRoot.path) || ".",
  ...
  const controlCommit = commitTrace(root, args, {
    ...
    files: [
      import_node_path34.default.relative(root, track.metadata_path),
      import_node_path34.default.relative(root, track.plan_path),
      import_node_path34.default.relative(root, trackPlanJsonPath(track)),
      import_node_path34.default.relative(root, completionJournalPath(track)),
      import_node_path34.default.relative(root, `${completionJournalPath(track)}l`),
      "cadre/events.jsonl"
    ],
    trackId: track.track_id,
    repo: ".",
...
function recordParallelWorkerUnlocked(root, track, args = {}) {
  ...
  const statePath = import_node_path35.default.join(track.dir, "parallel_state.json");
  ...
  const controlCommit = shouldTrace ? commitTrace(root, args, {
    ...
    before: traceBefore,
    files: [
      import_node_path35.default.relative(root, statePath)
    ],
    trackId: track.track_id,
  ...
  return {
    ok: controlCommit.ok !== false,
    track_id: track.track_id,
    state_path: import_node_path35.default.relative(root, statePath),
    worker: nextWorker,
    completion,
    control_commit: controlCommit,
    summary: {
...
function parallelStatePath(track) {
  return import_node_path35.default.join(track.dir, "parallel_state.json");
}
...
function isManifestChange(file) {
  const base = import_node_path36.default.basename(file).toLowerCase();
  return [
    "package.json",
    "package-lock.json",
    "pnpm-lock.yaml",
...
function fileStem(file) {
  return import_node_path36.default.basename(file).replace(/\.(test|spec|_test)\.[^.]+$/i, "").replace(/\.[^.]+$/, "");
}
...
function parallelSetupWorkers(root, track, args = {}) {
  ...
  const workers = asArray(wave.workers).map((rawWorker) => {
    ...
    const worktree = import_node_path36.default.resolve(root, ".worktrees", track.track_id, safeName(repo), safeName(worker.task_key));
...
function workflowArchive(root, args = {}) {
  ...
  const traceBefore = beginTrace(root);
  const archived = [];
  const archiveRoot = import_node_path37.default.join(root, "cadre", "archive");
  import_node_fs19.default.mkdirSync(archiveRoot, { recursive: true });
  for (const track of tracks) {
    const target = import_node_path37.default.join(archiveRoot, track.track_id);
    ...
    archived.push({ track_id: track.track_id, ok: true, path: import_node_path37.default.relative(root, target) });
  }
  const regen = regenIndex(root);
  const controlCommit = commitTrace(root, args, {
    kind: "control",
...
function workflowHandoff(root, args = {}) {
  ...
  const handoffPath = import_node_path37.default.join(track.dir, "HANDOFF.md");
  const handoffJsonPath = trackHandoffJsonPath(track);
  const handoffJson = markdownDocJson("handoff", text, { track_id: trackId });
  const reviewFiles = [
    jsonReviewFile(import_node_path37.default.relative(root, handoffJsonPath), "Track handoff canonical", "handoffText", handoffJson),
    textReviewFile(
      import_node_path37.default.relative(root, handoffPath),
      "Track handoff",
      "handoff.json",
      withGeneratedMarker(import_node_path37.default.relative(root, handoffJsonPath), "cadre.handoff.v1", renderMarkdownDoc(handoffJson, `Handoff: ${trackId}`))
    )
  ];
  const reviewArtifacts = reviewArtifactsFromFiles(reviewFiles);
  ...
  const base = {
    ...
    track_id: trackId,
    track_context: context,
    handoff_path: import_node_path37.default.relative(root, handoffPath),
    human_review: humanReview,
    review_artifacts: reviewArtifacts,
    review_bundle: reviewBundle,
    warnings
  ...
  if (args.execute === true) {
    ...
    import_node_fs19.default.writeFileSync(handoffPath, withGeneratedMarker(import_node_path37.default.relative(root, handoffJsonPath), "cadre.handoff.v1", renderMarkdownDoc(handoffJson, `Handoff: ${trackId}`)));
  }
  ...
  const message = appendCadreMessage(root, "outbox", {
    ...
    handoff_path: import_node_path37.default.relative(root, handoffPath),
    handoff_json_path: import_node_path37.default.relative(root, handoffJsonPath)
  });
  const event = appendCadreEvent(root, {
...
function refreshedPatternsArtifacts(root) {
  const jsonlPath = import_node_path38.default.join(root, "cadre", "patterns.jsonl");
  ...
  const projectionPath = import_node_path38.default.join(root, "cadre", "patterns.md");
  return {
    files: [
      plainReviewFile(import_node_path38.default.relative(root, jsonlPath), "Refreshed project patterns canonical", "refresh:patterns", jsonl),
      textReviewFile(import_node_path38.default.relative(root, projectionPath), "Refreshed project patterns projection", "cadre/patterns.jsonl", projection)
    ],
    jsonlPath,
    projectionPath,
    jsonl,
...
function workflowRefresh(root, args = {}) {
  ...
  if (args.execute === true) {
    ...
    if (refreshed) {
      ...
      patterns = {
        ok: true,
        path: import_node_path38.default.relative(root, refreshed.jsonlPath),
        projection: import_node_path38.default.relative(root, refreshed.projectionPath),
...
function releaseArtifactPlan(root, args = {}) {
  ...
  const releaseDir = import_node_path39.default.join(root, "cadre", "releases");
  const releaseSlug = safeName(version);
  const releaseMd = import_node_path39.default.join(releaseDir, `${releaseSlug}.md`);
  const releaseJson = import_node_path39.default.join(releaseDir, `${releaseSlug}.json`);
...
function releaseReviewFiles(root, plan) {
  return [
    textReviewFile(
      import_node_path39.default.relative(root, plan.releaseMd),
      "Release notes",
      "releaseNotes",
...
`
    ...
    jsonReviewFile(
      import_node_path39.default.relative(root, plan.releaseJson),
      "Release metadata",
      "releaseMetadata",
      plan.metadata
    )
...
function workflowRelease(root, args = {}) {
  ...
  const base = {
    ...
    release_artifacts: [import_node_path39.default.relative(root, plan.releaseMd), import_node_path39.default.relative(root, plan.releaseJson)],
    git_actions: plan.gitActions,
    human_review: humanReview,
    review_artifacts: reviewArtifacts,
    review_bundle: reviewBundle,
...
`);
  ...
  const indexPatch = patchJsonFile(import_node_path39.default.join(root, "cadre", "setup_state.json"), (current) => {
    current.last_release = {
      version: plan.version,
      path: import_node_path39.default.relative(root, plan.releaseMd),
      metadata: import_node_path39.default.relative(root, plan.releaseJson),
...
function workflowRevise(root, args = {}) {
  ...
  if (track && revisedSpec) {
    reviewFiles.push(jsonReviewFile(import_node_path39.default.relative(root, trackSpecJsonPath(track)), "Revised track spec canonical", "spec", revisedSpec));
    reviewFiles.push(textReviewFile(
      import_node_path39.default.relative(root, track.spec_path),
      "Revised track spec",
      "spec.json",
      withGeneratedMarker(import_node_path39.default.relative(root, trackSpecJsonPath(track)), "cadre.spec.v1", renderSpecMarkdown(revisedSpec))
    ));
  }
  if (track && revisedPlan) {
    reviewFiles.push(jsonReviewFile(import_node_path39.default.relative(root, trackPlanJsonPath(track)), "Revised track plan canonical", "plan", revisedPlan));
    reviewFiles.push(textReviewFile(
      import_node_path39.default.relative(root, track.plan_path),
      "Revised track plan",
      "plan.json",
      withGeneratedMarker(import_node_path39.default.relative(root, trackPlanJsonPath(track)), "cadre.plan.v1", renderPlanMarkdown(revisedPlan))
    ));
  }
  const reviewArtifacts = reviewArtifactsFromFiles(reviewFiles);
  ...
  const writeResult = withTrackLock(root, track.track_id, () => {
    ...
    if (revisedSpec) {
      ...
      import_node_fs21.default.writeFileSync(track.spec_path, withGeneratedMarker(import_node_path39.default.relative(root, trackSpecJsonPath(track)), "cadre.spec.v1", renderSpecMarkdown(revisedSpec)));
      written.push(import_node_path39.default.relative(root, trackSpecJsonPath(track)));
      written.push(import_node_path39.default.relative(root, track.spec_path));
    }
    if (revisedPlan) {
      writeJsonEnsured(trackPlanJsonPath(track), revisedPlan);
      import_node_fs21.default.writeFileSync(track.plan_path, withGeneratedMarker(import_node_path39.default.relative(root, trackPlanJsonPath(track)), "cadre.plan.v1", renderPlanMarkdown(revisedPlan)));
      written.push(import_node_path39.default.relative(root, trackPlanJsonPath(track)));
      written.push(import_node_path39.default.relative(root, track.plan_path));
    }
...
function workflowSetup(root, args = {}) {
  ...
  const cadreDir = import_node_path40.default.join(root, "cadre");
  ...
  const writeText = (relativePath, text) => {
    const file = import_node_path40.default.join(cadreDir, relativePath);
    if (fileExists(file) && !force) {
      skipped.push(import_node_path40.default.relative(root, file));
      return;
    }
    import_node_fs22.default.mkdirSync(import_node_path40.default.dirname(file), { recursive: true });
    import_node_fs22.default.writeFileSync(file, text);
    written.push(import_node_path40.default.relative(root, file));
  };
  ...
  import_node_fs22.default.mkdirSync(import_node_path40.default.join(cadreDir, "tracks"), { recursive: true });
  import_node_fs22.default.mkdirSync(import_node_path40.default.join(cadreDir, "archive"), { recursive: true });
...
function publicationLedger(root, workflow, track, args, data) {
  ...
  const file = import_node_path41.default.join(root, "cadre", "operations", "publication.jsonl");
  appendJsonl(file, entry);
  const controlCommit = commitTrace(root, args, {
    kind: "control",
    workflow,
    ...
    before,
    files: [
      import_node_path41.default.relative(root, file)
    ],
    trackId: track.track_id,
    note: entry
  });
  return { ok: controlCommit.ok !== false, path: import_node_path41.default.relative(root, file), entry, control_commit: controlCommit };
}
...
if (["cadre-job-runner.js", "cadre-job-runner.ts"].includes(import_node_path42.default.basename(process.argv[1] || ""))) {
...
function languageId(file, server) {
  const ext = import_node_path43.default.extname(file);
  const basename = import_node_path43.default.basename(file).toLowerCase();
...
${body}`);
  ...
  async initialize() {
    await this.request("initialize", {
      ...
      workspaceFolders: [{ uri: (0, import_node_url.pathToFileURL)(this.root).href, name: import_node_path44.default.basename(this.root) }]
    });
    this.notify("initialized", {});
  }
  open(file) {
    const abs = import_node_path44.default.join(this.root, file);
  ...
  async documentSymbols(file) {
    return this.request("textDocument/documentSymbol", {
      textDocument: { uri: (0, import_node_url.pathToFileURL)(import_node_path44.default.join(this.root, file)).href }
    });
  }
  ...
  diagnostics(file) {
    return this.publishedDiagnostics.get((0, import_node_url.pathToFileURL)(import_node_path44.default.join(this.root, file)).href) || [];
  }
  async shutdown() {
    try {
      await withTimeout(
...
function changedEntries(root, base, head) {
  return runGit(root, ["diff", "--name-status", "--find-renames", `${base}...${head}`]).split(/\r?\n/).map((line) => line.trim()).filter(Boolean).map((line) => {
    ...
    return {
      ...
      path: file || "",
      oldPath,
      exists: file ? import_node_fs24.default.existsSync(import_node_path45.default.join(root, file)) : false
    };
...
function serverFileMatch(file, server) {
  ...
  const ext = import_node_path46.default.extname(file);
  const basename = import_node_path46.default.basename(file).toLowerCase();
...
function scanTextReferences(root, symbol, changedPathSet, server) {
  ...
  function visit(dir) {
    ...
    for (const entry of entries) {
      const full = import_node_path46.default.join(dir, entry.name);
      if (shouldIgnore(root, full, entry.name)) continue;
      if (entry.isDirectory()) {
        visit(full);
      ...
      if (changedPathSet.has(import_node_path46.default.resolve(full))) continue;
      let stat;
      try {
      ...
      for (let i = 0; i < lines.length; i += 1) {
        ...
        results.push({
          file: full,
          relativeFile: normalizeRel(import_node_path46.default.relative(root, full)),
          line: i + 1,
...
function lspRefToLocation(root, ref) {
  ...
  try {
    ...
    return {
      file,
      relativeFile: normalizeRel(import_node_path46.default.relative(root, file)),
...
function nearbyFileHints(root, files) {
  ...
  for (const file of files || []) {
    let dir = import_node_path46.default.dirname(import_node_path46.default.join(root, file));
    while (dir.startsWith(root)) {
      for (const name of manifestNames) {
        const candidate = import_node_path46.default.join(dir, name);
        if (import_node_fs25.default.existsSync(candidate)) manifests.add(normalizeRel(import_node_path46.default.relative(root, candidate)));
      }
      if (dir === root) break;
      dir = import_node_path46.default.dirname(dir);
    }
    const parsed = import_node_path46.default.parse(file);
    const candidates = [
      import_node_path46.default.join(parsed.dir, `${parsed.name}.test${parsed.ext}`),
      import_node_path46.default.join(parsed.dir, `${parsed.name}.spec${parsed.ext}`),
      import_node_path46.default.join(parsed.dir, `${parsed.name}_test${parsed.ext}`),
      import_node_path46.default.join("test", file),
      import_node_path46.default.join("tests", file)
    ];
    for (const candidate of candidates) {
      if (import_node_fs25.default.existsSync(import_node_path46.default.join(root, candidate))) tests.add(normalizeRel(candidate));
    }
  }
  return {
...
async function runReview(options = {}) {
  ...
  const configPath = import_node_path47.default.resolve(root, args.config);
  ...
  for (const entry of entries) {
    if (entry.path) changedSet.add(import_node_path47.default.resolve(root, entry.path));
    if (entry.oldPath) changedSet.add(import_node_path47.default.resolve(root, entry.oldPath));
  }
    ...
    try {
      ...
      for (const file of openFiles) {
        ...
        for (const candidate of candidates) {
          ...
          const externalRefs = refs.map((ref) => lspRefToLocation(root, ref)).filter((ref) => ref !== null).filter((ref) => !changedSet.has(import_node_path47.default.resolve(ref.file))).filter((ref) => !isIgnoredFile(root, ref.relativeFile));
      ...
      for (const candidate of allCandidates.values()) {
        if (candidate.changeType !== "removed") continue;
        if (candidate.changedFile && import_node_fs26.default.existsSync(import_node_path47.default.join(root, candidate.changedFile))) continue;
        const refs = scanTextReferences(root, candidate.name, changedSet, server);
        if (refs.length > 0) {
          findings.push(externalReferenceFinding(server, candidate, refs, "text"));
        }
...
// src/cadre-lsp-review.ts
if (["cadre-lsp-review.js", "cadre-lsp-review.ts"].includes(import_node_path48.default.basename(process.argv[1] || ""))) {
  runCli().catch((error) => {
...
if (["cadre-lsp-daemon.js", "cadre-lsp-daemon.ts"].includes(import_node_path49.default.basename(process.argv[1] || ""))) {
  runLspDaemon();
}
...
function parseArgs2(argv) {
  ...
  const root = import_node_path50.default.resolve(args.root);
  return { ...args, root, configPath: import_node_path50.default.resolve(root, args.config) };
}
...
function saveConfig(configPath, config) {
  import_node_fs27.default.mkdirSync(import_node_path50.default.dirname(configPath), { recursive: true });
...
function workspaceFolders(root) {
  ...
  const reposPath = import_node_path50.default.join(root, "cadre", "repos.json");
  let repos = {};
  try {
...
function runCli2() {
  ...
  const result = {
    root: args.root,
    config: import_node_path50.default.relative(args.root, args.configPath),
  ...
  if (written) {
    console.log(`Updated ${import_node_path50.default.relative(args.root, args.configPath)}; added: ${added.join(", ") || "none"}.`);
...
// src/cadre-lsp-setup.ts
if (["cadre-lsp-setup.js", "cadre-lsp-setup.ts"].includes(import_node_path51.default.basename(process.argv[1] || ""))) {
  try {
    runCli2();
  } catch (error) {
...
function resourceRead(uri, deps) {
  ...
  if (resource.base === "cadre://skill-contract") value = skillContract();
    ...
    if (resource.base === "cadre://team-board") value = deps.core.teamBoard(root);
      ...
      value = context.ok === false || !planPath ? context : deps.core.parsePlanFile(import_node_path52.default.resolve(root, planPath));
...
function repoReviewTargets(deps, root, args) {
  ...
  return rawTargets.map((entry) => {
    ...
    return {
      repo,
      path: rel,
      cwd: import_node_path53.default.isAbsolute(rel) ? rel : import_node_path53.default.resolve(root, rel),
...
async function warmLspReview(deps, root, args) {
  ...
  const rawConfig = asOptionalString(args.config) || import_node_path53.default.join(root, "cadre", "lsp.json");
  const config = import_node_path53.default.isAbsolute(rawConfig) ? rawConfig : import_node_path53.default.resolve(root, rawConfig);
...
function trackPacket(deps, args) {
  ...
  if (action === "parse_plan") {
    ...
    return envelope(deps.core.parsePlanFile(import_node_path54.default.resolve(root, track.plan_json_path)));
  }
...
var JobManager = class {
  ...
  jobDir(root) {
    return import_node_path55.default.join(root, "cadre", "jobs");
  }
  jobPath(root, id) {
    return import_node_path55.default.join(this.jobDir(root), `${id}.json`);
  }
  serialize(job) {
    return {
      id: job.id,
      ...
      signal: job.signal,
      artifact_path: job.artifact_path || import_node_path55.default.relative(job.root, this.jobPath(job.root, job.id))
    };
  }
  persist(job) {
    try {
      ...
      job.artifact_path = import_node_path55.default.relative(job.root, artifactPath);
...
`);
  ...
  summaryFromState(job, persisted = false) {
    ...
    return {
      ...
      artifact_path: typeof job.artifact_path === "string" ? job.artifact_path : root && id ? import_node_path55.default.relative(root, this.jobPath(root, id)) : null,
      persisted,
...
function hasCadreDirectory(dir) {
  return isDirectory(import_node_path56.default.join(dir, "cadre"));
}
function isCadreStateDirectory(dir) {
  return [
    ...
    "config.json",
    "repos.json"
  ].some((name) => import_node_fs30.default.existsSync(import_node_path56.default.join(dir, name))) || isDirectory(import_node_path56.default.join(dir, "tracks"));
}
function hasCadreProjectState(dir) {
  return hasCadreDirectory(dir) && isCadreStateDirectory(import_node_path56.default.join(dir, "cadre"));
}
function normalizePathCandidate(value) {
  ...
  candidate = import_node_path56.default.resolve(candidate);
  try {
    if (import_node_fs30.default.existsSync(candidate) && !import_node_fs30.default.statSync(candidate).isDirectory()) candidate = import_node_path56.default.dirname(candidate);
  } catch {
    return null;
  }
  return candidate;
...
function findCadreRoot(start) {
  ...
  while (true) {
    ...
    if (import_node_path56.default.basename(dir) === "cadre" && isCadreStateDirectory(dir)) return import_node_path56.default.dirname(dir);
    const parent = import_node_path56.default.dirname(dir);
