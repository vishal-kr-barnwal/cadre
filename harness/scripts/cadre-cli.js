...
var PROTOCOL_VERSION = SUPPORTED_PROTOCOL_VERSIONS[0];
...
`,
  ...
  try {
    const responses = result.stdout.split(/\r?\n/).filter((entry) => entry.trim()).map((entry) => asJsonObject(JSON.parse(entry)));
...
function checkCommandSkills(platform, pluginRoot, commandSkills) {
  ...
  for (const [command, files] of Object.entries(commandSkills)) {
    ...
    for (const [relative, expected] of Object.entries(files)) {
      const file = import_node_path3.default.join(pluginRoot, "skills", command, ...relative.replace(/\\/g, "/").split("/"));
      if (!import_node_fs2.default.existsSync(file)) {
        errors.push(`missing ${file}`);
        continue;
      }
      if (!import_node_fs2.default.lstatSync(file).isFile()) {
        errors.push(`invalid Cadre command skill file ${file}`);
        continue;
      }
      const actual = import_node_fs2.default.readFileSync(file, "utf8");
      const normalizedExpected = expected.endsWith("\n") ? expected : `${expected}
`;
      if (actual !== normalizedExpected) errors.push(`stale Cadre command skill ${file}`);
    }
    if (import_node_fs2.default.existsSync(commandRoot)) {
      const visit = (directory, relativeRoot = "") => {
        for (const entry of import_node_fs2.default.readdirSync(directory, { withFileTypes: true })) {
          const relative = import_node_path3.default.posix.join(relativeRoot, entry.name);
          if (entry.isDirectory()) {
            if (expectedDirectories.has(relative)) visit(import_node_path3.default.join(directory, entry.name), relative);
            else errors.push(`unexpected Cadre command skill directory ${import_node_path3.default.join(commandRoot, ...relative.split("/"))}`);
          } else if (!expectedFiles.has(relative)) {
            errors.push(`unexpected Cadre command skill file ${import_node_path3.default.join(commandRoot, ...relative.split("/"))}`);
          }
        }
      };
      visit(commandRoot);
  ...
  const skillsRoot = import_node_path3.default.join(pluginRoot, "skills");
  const expectedCommands = new Set(Object.keys(commandSkills));
  if (import_node_fs2.default.existsSync(skillsRoot)) {
    for (const entry of import_node_fs2.default.readdirSync(skillsRoot, { withFileTypes: true })) {
      if (!entry.isDirectory() || !expectedCommands.has(entry.name)) {
        errors.push(`unexpected Cadre command skill ${import_node_path3.default.join(skillsRoot, entry.name)}`);
      }
    }
  }
  return errors;
...
function writeCommandSkills(pluginRoot, commandSkills) {
  for (const [command, files] of Object.entries(commandSkills)) {
    ...
    for (const [relative, content] of Object.entries(files)) {
      ...
      writeText(import_node_path4.default.join(pluginRoot, "skills", command, ...normalized.split("/")), content);
    }
  }
}
...
function renderWorkflowCommandSkills(template, workflows, options = {}) {
  ...
  for (const workflow of workflows) {
    ...
    unique.add(workflow);
    const content = template.replaceAll("{{command}}", workflow).replaceAll("{{workflow}}", workflow);
    if (content.includes("{{")) {
...
// src/cli.ts
var commandSkillTemplate = typeof __CADRE_COMMAND_SKILL_TEMPLATE__ === "string" ? __CADRE_COMMAND_SKILL_TEMPLATE__ : "";
