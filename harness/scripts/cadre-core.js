...
function workflowRefresh(root, args = {}) {
  ...
  const mutatingRefresh = reviewFiles2.length > 0 || lspSelected || projectionsSelected;
  const reviewArtifacts = reviewArtifactsFromFiles(reviewFiles2);
  if (lspSelected) reviewArtifacts.push(...setupLspReviewArtifacts(args, true));
  const reviewBundle = workflowReviewBundle(root, "refresh", args, reviewFiles2);
  const approval = reviewFiles2.length > 0 ? stagedApprovalState(root, "refresh", args, refreshApprovalStages(levels), reviewFiles2, {
    selected_levels: levels,
  ...
  const lsp = args.execute === true && lspSelected ? lspSetup(root, { ...args, execute: true }) : lspPreview;
...
function releaseArtifactPlan(root, args = {}) {
  ...
  const releaseDir = import_node_path51.default.join(root, "cadre", "releases");
  const releaseSlug = safeName(version);
  const releaseMd = import_node_path51.default.join(releaseDir, `${releaseSlug}.md`);
  const releaseJson = import_node_path51.default.join(releaseDir, `${releaseSlug}.json`);
...
`
  ...
  const notes = withGeneratedMarker(
    import_node_path51.default.relative(root, releaseJson),
    "cadre.release.v1",
    notesBody,
    {
      canonicalContent: `${JSON.stringify(metadata, null, 2)}
`,
      projection: import_node_path51.default.relative(root, releaseMd)
    }
  );
...
function releaseReviewFiles(root, plan) {
  return documentReviewPair(
    "release_notes",
    jsonReviewFile(
      import_node_path51.default.relative(root, plan.releaseJson),
      "Release metadata",
      "releaseMetadata",
      plan.metadata
    ),
    textReviewFile(
      import_node_path51.default.relative(root, plan.releaseMd),
      "Release notes",
      "releaseNotes",
...
function workflowRelease(root, args = {}) {
  ...
  const base = {
    ...
    release_artifacts: [import_node_path51.default.relative(root, plan.releaseMd), import_node_path51.default.relative(root, plan.releaseJson)],
    git_actions: plan.gitActions,
    approval,
    human_review: humanReview,
    review_artifacts: stageReviewArtifacts,
  ...
  const reusedReviewFiles = new Set(asStringArray(reviewValidation.files));
  const traceBefore = beginTrace(root);
  import_node_fs29.default.mkdirSync(plan.releaseDir, { recursive: true });
  if (!reusedReviewFiles.has(import_node_path51.default.relative(root, plan.releaseMd))) {
    import_node_fs29.default.writeFileSync(plan.releaseMd, plan.notes.endsWith("\n") ? plan.notes : `${plan.notes}
`);
  }
  if (!reusedReviewFiles.has(import_node_path51.default.relative(root, plan.releaseJson))) writeJson(plan.releaseJson, plan.metadata);
  const indexPatch = patchJsonFile(import_node_path51.default.join(root, "cadre", "setup_state.json"), (current) => {
    current.last_release = {
      version: plan.version,
      path: import_node_path51.default.relative(root, plan.releaseMd),
      metadata: import_node_path51.default.relative(root, plan.releaseJson),
...
function workflowRevise(root, args = {}) {
  ...
  if (track && revisedSpec) {
    reviewFiles2.push(...documentReviewPair(
      "spec",
      jsonReviewFile(import_node_path51.default.relative(root, trackSpecJsonPath(track)), "Revised track spec canonical", "spec", revisedSpec),
      textReviewFile(
        import_node_path51.default.relative(root, track.spec_path),
        "Revised track spec",
        "spec.json",
        withGeneratedMarker(import_node_path51.default.relative(root, trackSpecJsonPath(track)), "cadre.spec.v1", renderSpecMarkdown(revisedSpec, import_node_path51.default.relative(root, trackSpecJsonPath(track))), { canonicalContent: `${JSON.stringify(revisedSpec, null, 2)}
`, projection: import_node_path51.default.relative(root, track.spec_path) })
      )
    ));
  }
  if (track && revisedPlan) {
    reviewFiles2.push(...documentReviewPair(
      "plan",
      jsonReviewFile(import_node_path51.default.relative(root, trackPlanJsonPath(track)), "Revised track plan canonical", "plan", revisedPlan),
      textReviewFile(
        import_node_path51.default.relative(root, track.plan_path),
        "Revised track plan",
        "plan.json",
        withGeneratedMarker(import_node_path51.default.relative(root, trackPlanJsonPath(track)), "cadre.plan.v1", renderPlanMarkdown(revisedPlan, import_node_path51.default.relative(root, trackPlanJsonPath(track))), { canonicalContent: `${JSON.stringify(revisedPlan, null, 2)}
`, projection: import_node_path51.default.relative(root, track.plan_path) })
      )
    ));
  }
  const reviewArtifacts = reviewArtifactsFromFiles(reviewFiles2);
  ...
  const writeResult = withTrackLock(root, track.track_id, () => {
    const written = [];
    if (revisedSpec) {
      if (!reusedReviewFiles.has(import_node_path51.default.relative(root, trackSpecJsonPath(track)))) writeJsonEnsured(trackSpecJsonPath(track), revisedSpec);
      if (!reusedReviewFiles.has(import_node_path51.default.relative(root, track.spec_path))) {
        import_node_fs29.default.writeFileSync(track.spec_path, withGeneratedMarker(import_node_path51.default.relative(root, trackSpecJsonPath(track)), "cadre.spec.v1", renderSpecMarkdown(revisedSpec, import_node_path51.default.relative(root, trackSpecJsonPath(track)))));
      }
      written.push(import_node_path51.default.relative(root, trackSpecJsonPath(track)));
      written.push(import_node_path51.default.relative(root, track.spec_path));
    }
    if (revisedPlan) {
      if (!reusedReviewFiles.has(import_node_path51.default.relative(root, trackPlanJsonPath(track)))) writeJsonEnsured(trackPlanJsonPath(track), revisedPlan);
      if (!reusedReviewFiles.has(import_node_path51.default.relative(root, track.plan_path))) {
        import_node_fs29.default.writeFileSync(track.plan_path, withGeneratedMarker(import_node_path51.default.relative(root, trackPlanJsonPath(track)), "cadre.plan.v1", renderPlanMarkdown(revisedPlan, import_node_path51.default.relative(root, trackPlanJsonPath(track)))));
      }
      written.push(import_node_path51.default.relative(root, trackPlanJsonPath(track)));
      written.push(import_node_path51.default.relative(root, track.plan_path));
    }
...
function styleGuideFiles(styleGuides) {
...
function lspPreviewPayload(root, recommendations, repos = null) {
  const existing = readJson(import_node_path52.default.join(root, "cadre", "lsp.json"), {});
...
function setupFinalReviewPlan(input) {
  ...
  const machineFiles = [
    machineReviewFile(
      ...
      "setup:native-state",
      appendRequiredLine(fileExists(import_node_path52.default.join(root, "cadre", ".gitignore")) ? import_node_fs30.default.readFileSync(import_node_path52.default.join(root, "cadre", ".gitignore"), "utf8") : "", "/local/")
    ),
...
`)
  ...
  if (gitattributesNeeded) {
    const attributesPath = import_node_path52.default.join(root, ".gitattributes");
    machineFiles.push(machineReviewFile(
      ".gitattributes",
      "Cadre merge attributes",
      "setup:gitattributes",
      appendRequiredLine(fileExists(attributesPath) ? import_node_fs30.default.readFileSync(attributesPath, "utf8") : "", "cadre/tracks/**/parallel_state.json merge=ours")
    ));
  }
  ...
  if (ciProvider && rawArgs6.writeCi !== false && rawArgs6.write_ci !== false) {
    ...
    if (!fileExists(import_node_path52.default.join(root, ciPath)) || rawArgs6.force === true) {
...
`)] : [];
  ...
  const reviewFiles2 = setupScopedReviewFiles(plannedCollection.cursor, currentReviewFiles, finalPlan?.reviewFiles || []);
...
function nextCall(root, workflow, result, resources, args) {
  ...
  const approval = asJsonObject(result.approval);
  if ((workflow === "setup" || workflow === "skill") && result.ok !== false && result.phase_state !== "pending_provider" && asStringArray(result.missing_payload).length === 0 && args.execute !== true && approval.cancelled !== true && asStringArray(approval.pending_stages).length === 0 && approval.approval_error == null && approval.session_id) {
    return {
      tool: "cadre_workflow",
      arguments: {
        root,
