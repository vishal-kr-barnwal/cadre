...
Call `project_status` and `state_validate` first. If the Cadre MCP is unavailable, stop without starting or advancing an archive batch.
...
7. After approval and before any move, call `template_get` for `project/archive-operation`, render `.cadre/operations/archive-<timestamp>.json`, and record the batch ID, ordered selection, base commit, expected commit, approved artifacts, approval time, checkpoint, and per-track/artifact progress.
