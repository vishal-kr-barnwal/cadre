...
4. If `.cadre/project.json` exists, read `setup`, its operation journal, Git status, and recent commits. If setup is incomplete, resume its last durable checkpoint using the workflow's interruption protocol; do not restart or overwrite it. Offer `$refresh` or `$status` only when setup is completed.
