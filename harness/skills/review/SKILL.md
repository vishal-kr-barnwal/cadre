...
2. Run relevant verification without mutating production behavior. Check evidence quality: recorded commands and results bind to the reviewed HEAD and state baseline and delta, a blocked check stands only as a blocker or an explicitly human-accepted risk, and negative claims are independently confirmed. For an operation track, compare recorded evidence with every planned preflight, rollout, and postflight capture, the monitoring baseline and success threshold across the observation window, and any abort or rollback decision. Missing or contradictory evidence is a finding; never complete from assumed external outcomes. Report findings by severity with file/line evidence, impact, reproduction, and proposed acceptance criteria. Do not write a bug file yet.
...
4. For v6 approved changes, include the findings, proposed final state, appended plan, reassessed learning and dependency context in `candidate_apply` prepare with `workflow: "review"`. Preserve completed execution and finding evidence. Under valid Autonomous authority bind the exact candidate to that authority; other modes require the human decision. Apply only the unchanged approved token. Commit `cadre(review): request changes for <track-id>` once with receipt trailers and reconcile before the next execution. For legacy recovery, retain the original candidate_inspect planValidations and SHA journal contract.
