...
3. Prepare one decision-ready proposal before asking for approval:
   ...
   - Inspection validates that target status without changing canonical state. Keep the canonical track `ready_for_review` and its completed execution unchanged until approval. If a staged state is included, its status must agree with `targetStatus`; an active execution still requires reconciliation at a safe boundary.
