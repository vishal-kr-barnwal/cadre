...
Record a ready diff as `awaiting_approval` with verification evidence, present it once, and proceed only after explicit approval. Bind integration approval to the `integration` prepare digest. After approval call the same tool with only `{ mode: "apply", proposalToken }`; changed content or Git/state drift requires a fresh prepare and decision.
