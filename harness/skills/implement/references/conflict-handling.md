...
In governed mode, present the exact resolution for approval. In phase/autonomous mode, apply only an unambiguous in-scope resolution; stop for clarification if product intent, scope, compatibility, or authority is material. Use standalone checkpoints to record recovery and the verified resulting integration. Never force-delete, reset, or silently choose a side.
