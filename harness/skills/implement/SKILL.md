...
At every required clarification or manual-verification boundary, show concise evidence and prefer `workflow_elicit`: use `clarification` for at most three questions and `approval` bound to the execution ID, node ID, and verified commit. Treat only an `approved` result as approval. If it returns `fallback_required`, ask the same short question once in chat; never request secrets or retry the form.
