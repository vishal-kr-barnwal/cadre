...
For a new execution, the human's `implement` invocation authorizes an exact execution start using the requested modes or their documented defaults. Call `execution_start_preview`, then pass only its opaque `proposalToken` to `execution_start_apply`. Do not add an execution-start approval prompt outside `governed`; report a concise start summary and proceed. The server derives the execution ID, timestamp, and current Git base. Generated bookkeeping is a deterministic consequence of the same authorization. For an existing `implement` operation, reconcile it instead of creating another execution.
...
- Phase and task worktrees are siblings. Use `worktree_create_preview`, then apply only its proposal token. The MCP derives both the task phase and correct parent HEAD, so omit `phaseId` and `baseCommit`. Record the returned absolute path/branch with a `start` checkpoint before spawning. In `phase` and `autonomous`, these deterministic mutations run without another approval.
...
2. Record that approval with `record_verification`, then call `execution_finish_preview`. It derives HEAD/time and atomically proposes plan commit markers, the completed journal, `ready_for_review` state, and `tracks.md`; pass only its proposal token to apply. Do not repair markers or the index separately, and do not ask for another approval.
