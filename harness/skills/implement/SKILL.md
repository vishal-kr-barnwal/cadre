...
- Run an irreversible or destructive step only as its approved plan task, under the persisted approval mode without an extra pause. First confirm its actual target and consequences match that task; a material difference is a scope change, so stop and route it through revise.
...
- Verify with the approved verification profile in `.cadre/tech-stack.md`; a legacy project without one uses repository instructions and existing scripts and records the exact commands. Verification evidence states the command or inspection, result, commit, baseline and delta, and any blocked check. Never report an unrun check as passing.
- For an operation track, humans perform external preflight, rollout, postflight, rollback, and recovery actions. Record the human-supplied evidence (operator, RFC 3339 timestamp, source/location or hash, and observed signal versus baseline and threshold) in the matching manual-verification checkpoint; never infer that an external action occurred. An abort-threshold breach blocks the phase until the human records the rollback or recovery decision.
