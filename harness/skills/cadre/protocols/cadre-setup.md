...
> `cadre-setup` is the only Cadre workflow that may begin before `cadre/` exists.
> The generated protocol still requires `cadre_project` with `action: "ping"` before setup starts.
> Project-scoped MCP calls begin as soon as this workflow creates or confirms
> `cadre/`.

0. **MCP runtime + diagnostic check:** Call `cadre_project` with `action: "ping"` before any project
   mutation. Then call `cadre_project` with `action: "doctor"` with the current working directory as
   `root`. `cadre_project` with `action: "doctor"` is valid even before `cadre/` exists; use it to detect
...
2. **Execute Based on Maturity:**
   ...
   **Hard prerequisite preflight (before project mutation):**
   - Call `cadre_project` with `action: "ping"`. If Cadre MCP is unavailable, HALT and ask the user to
   ...
   **IF GREENFIELD:**
   ...
   - Upon response:
     ...
     - Call `cadre_project` with `action: "root"` with `root` (the current project root or any path
       inside it) and use the returned root for later project-scoped MCP calls.
...
2. **Persist the choice** in `cadre/setup_state.json` (merge, don't overwrite
   ...
   - Call `cadre_project` with `action: "root"` with `root` if this is the first point where
     `cadre/` exists in a brownfield setup path.
...
5. **MCP polyrepo sanity check:** Call `cadre_project` with `action: "polyrepo_preflight"` with `root`.
   Surface returned errors immediately and let the user fix the manifest or
   submodules before continuing.

6. **Commit State** (merge keys):
   ```json
   {"last_successful_step": "2.0b_repos_manifest", "topology": "polyrepo"}
   ```

7. **Continue:** Proceed to **Section 2.1**.
...
5. **Seed the Tracks Index:** Create `cadre/tracks.md` as a **derived index**
   ...
   This list is regenerated by MCP `cadre_mutate` with `action: "regen_index"`; edit `metadata.json`, not the markers below.
   ...
   After writing the seed file, call `cadre_mutate` with `action: "regen_index"` with `root` to verify the
   generated region is valid and idempotent. Require `ok: true`; halt and surface
   the MCP error if it fails.
...
2. **If Beads Available, Enable Full Integration:**
   - Do not ask for a Beads mode. Cadre uses full Beads integration by default so
     the task graph is part of the shared control plane.

3. **Initialize Beads:**
   - If `.beads/` already exists: treat Beads as initialized and continue.
   - Otherwise run:
     ```bash
   ...
   - Create `cadre/beads.json` from the template: copy
     ...
     **Copy the template verbatim** — it is the canonical schema; do not hand-write
     the keys here (an inline copy would silently drift from `templates/beads.json`).
     The template's `"mode": "normal"` is the only supported Cadre setup mode.
   - Announce: "Beads integration enabled in full mode."
...
3a. **Configure `.gitattributes` for Beads:**
   ...
   - **Register the `ours` merge driver (CRITICAL) — runs for ALL Beads-enabled
     Cadre projects (monorepo + polyrepo, local + shared), wherever
...
3. **Write `cadre/config.json`:** copy `<TEMPLATES_DIR>/config.json` (resolved in
   ...
   - `sync_mode`: `"shared"` (A) or `"local"` (B). This is the master gate for all
     team/shared behaviors across workflows; `"local"` keeps today's behavior.
...
0. **Gitignore agent-local state:** create or extend `cadre/.gitignore` so
   ...
   - **ALWAYS ignore** (both monorepo and polyrepo, any sync mode):
     ...
     refresh_state.json
     .locks/
     tracks/**/completion_journal.json
     ```
...
1. **Final MCP verification:** Call `cadre_project` with `action: "root"` with `root` once more,
   then call `cadre_project` with `action: "doctor"` with that resolved root. If any tracks were created
   during a resumed/custom setup path, also call `cadre_status` with `action: "team"` to verify the
...
4. **Publish control plane:** this is the first publish of the control plane.
   After the setup commit, call MCP `cadre_project` with `action: "sync_control_plane"` with
