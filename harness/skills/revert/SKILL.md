...
Call `project_status` first with `view: "implementation"` and `trackId` for active work, otherwise `view: "track"`; use its embedded structured validation and do not repeat `state_validate` at command entry. If the Cadre MCP is unavailable, stop without beginning the revert.
...
2. Build the exact additive revert proposal, call `candidate_stage_prepare` for `revert-<track-id>`, and render every proposed state/plan/learning reconciliation file beneath the returned stage. The Git reversal remains an exact operation/commit proposal rather than a staged product mutation.
3. Call `candidate_inspect` once for the staged reconciliation set, supplying the post-revert `targetStatus` when it includes `plan.md`. Present the manifest, graph result, Git operations, expected diff, downstream reconciliation, verification, index, and commits as one authorization envelope bound to its digest. Never rewrite public history by default.
