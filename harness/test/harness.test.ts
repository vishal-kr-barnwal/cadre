...
function agentVisibleResult(result: Awaited<ReturnType<Client["callTool"]>>): Record<string, unknown> {
  ...
  if (result.isError) {
    assert.equal(immediate.structuredContent, undefined);
    assert.equal(immediate.content.length, 1, "errors use one client-visible text representation");
    return JSON.parse(immediate.content[0]!.text!);
  }
  assert.deepEqual(immediate.content, [], "structured clients must not receive duplicate text");
  assert.ok(immediate.structuredContent);
...
function writeWorktreeJournal(
  ...
  const trackRoot = dirname(dirname(path));
  const planBody = "# Plan\n- Spec revision: 1\n- Plan revision: 1\n## Phase 1: Test\n- Phase dependencies: none\n- [ ] T1.1 Test task\n  - Task dependencies: none\n- Phase completion commit: pending\n\n## Phase 2: Track-level User Manual Verification\n- [ ] T2.1 User Manual Verification\n- Phase completion commit: pending\n";
  writeFileSync(join(trackRoot, "plan.md"), planBody);
  const graph = parsePlanContent(planBody, "fixture");
  validatePlanGraph("fixture", graph, "in_progress", []);
  const graphDigest = graph.digest;
  writeFileSync(join(trackRoot, "state.json"), JSON.stringify({ schemaVersion: 1, trackId, status: "in_progress", operation: {
    action: "implement", executionId, journal: `executions/execution-${executionId}.json`, graphDigest, planRevision: 1, approvalMode: approvalMode ?? "governed"
  } }));
  writeFileSync(path, `${JSON.stringify({
    ...
    nodes: {
      ...
      P2: { ...node("P2", "phase", "P2", "pending"), dependencies: ["P1"] },
      "T2.1": { ...node("T2.1", "manual-verification", "P2", "pending"), dependencies: ["P1"] }
    }
  }, null, 2)}\n`);
  const relativePath = relative(projectRoot, path);
  if (gitText(projectRoot, ["ls-files", "--", relativePath]) !== relativePath) {
    gitText(projectRoot, ["add", relative(projectRoot, trackRoot)]);
...
test("setup commit recording verifies the approved file manifest against Git", () => {
  ...
  const applied = applyProjectInitCandidate(input, preview.digest);
  execFileSync("git", ["init", "-b", "main"], { cwd: projectRoot });
  gitText(projectRoot, ["config", "user.name", "Cadre Test"]);
  gitText(projectRoot, ["config", "user.email", "cadre@example.test"]);
  gitText(projectRoot, ["config", "commit.gpgsign", "false"]);
  ...
  gitText(projectRoot, ["commit", "-m", "cadre(create): initialize project harness", ...applied.receipt!.trailers.flatMap((trailer) => ["-m", trailer])]);
  ...
  assert.equal(state.setup.commit, `op:${applied.receipt!.operationId}`);
...
test("clean review completion previews and applies its exact state and derived index together", () => {
  ...
  assert.equal(preview.state.reviewCycles?.at(-1)?.reviewEvidence, input.approval);
  assert.equal(preview.state.reviewCycles?.at(-1)?.approvalConfirmation, undefined);
  ...
  assert.equal(retriedState.reviewCycles[0].approvalConfirmation.status, "approved");
  assert.equal(retriedState.reviewCycles[0].approvalConfirmation.proposalDigest, fresh.digest);
  assert.equal(retriedState.reviewCycles[0].approval, undefined);
...
`);
  ...
  const rotatedNodePath = "/opt/cadre/node-updated";
  const zedRotated = configureZedCadre(zedSettings, rotatedNodePath, mcpPath, true);
  assert.equal(zedRotated.changed, true);
  assert.equal(zedRotated.serverChanged, true);
  assert.equal(zedRotated.permissionsChanged, false);
  assert.match(readFileSync(zedSettings, "utf8"), /"command": "\/opt\/cadre\/node-updated"/);
...
test("compiled MCP exposes versioned templates and initializes projects without copied runtime", async () => {
  ...
  try {
    const tools = await client.listTools();
    assert.equal(tools.tools.length, 25);
    assert.ok(Buffer.byteLength(JSON.stringify(tools.tools)) <= 80 * 1024);
    for (const name of [
      "workflow_elicit", "template_get_many", "styleguide_resolve", "context_read", "project_status",
    ...
    assert.equal(checkpointSchema.properties?.action?.oneOf?.length, 8);
    ...
    assert.equal(textContents.length, 0);
...
`;
    ...
    assert.equal(
      (agentVisibleResult(stagedImplementationStatus) as { error?: { code?: string } }).error?.code,
    ...
    assert.equal(
      (agentVisibleResult(invalidCanonicalStatus) as { error?: { code?: string } }).error?.code,
      "TRACK_STATE_INVALID"
    );
    ...
    assert.match(
      (agentVisibleResult(illegalTransition) as { error?: { message?: string } }).error?.message ?? "",
    ...
    const rejectedLegacyMutation = await client.callTool({
      ...
      arguments: { projectRoot: legacyRoot, trackId: "legacy-track" }
    });
    ...
    writeFileSync(
      ...
      readFileSync(join(providerRoot, "init/workflow.md"), "utf8")
    );
    ...
    const restartedClient = new Client({ name: "codex_cli_rs", version: "1.0.0" });
    ...
    assert.equal(initializedProject.setup.operation, null);
    const initReceipt = JSON.parse(readFileSync(join(projectRoot, ".cadre/receipts", initializedProject.setup.commit.slice(3) + ".json"), "utf8"));
    assert.equal(initReceipt.status, "completed");
    assert.ok(initReceipt.artifacts.length > files.length);
    assert.equal(
...
test("compiled MCP integration pauses only when the execution is governed", async () => {
  ...
  try {
    for (const approvalMode of ["phase", "governed"] as const) {
      ...
      writePlannedTrack(projectRoot, trackId);
      gitText(projectRoot, ["add", ".cadre"]);
      gitText(projectRoot, ["commit", "-m", "cadre(test): record approved memory"]);
      ...
      if (approvalMode === "phase") {
        // Simulate a crash after Git merged but before its journal receipt, then a runtime upgrade.
        const journalPath = join(projectRoot, ".cadre", "tracks", trackId, "executions", `execution-${executionId}.json`);
        const interrupted = JSON.parse(readFileSync(journalPath, "utf8"));
        interrupted.nodes["T1.1"].status = "committed";
        interrupted.nodes["T1.1"].mergeCommit = null;
        interrupted.nodes["T1.1"].handoff = { decisions: ["Preserve merged behavior"], failedApproaches: [], openQuestions: [], nextAction: "Record existing integration", sources: [] };
        writeFileSync(journalPath, JSON.stringify(interrupted));
        const projectPath = join(projectRoot, ".cadre/project.json");
        const project = JSON.parse(readFileSync(projectPath, "utf8"));
        project.runtimeVersion = "3.6.0"; project.templateSetVersion = "v2";
        writeFileSync(projectPath, JSON.stringify(project));
        gitText(projectRoot, ["add", ".cadre/project.json"]);
        gitText(projectRoot, ["commit", "-m", "cadre(test): preserve legacy project version"]);
        const beforeRecovery = gitText(projectRoot, ["rev-parse", "HEAD"]);
        const recovered = await client.callTool({ name: "integration", arguments: { request: { mode: "prepare", ...scope } } });
        assert.equal(recovered.isError, undefined, JSON.stringify(recovered));
        assert.equal(agentVisibleResult(recovered).commandStatus, "applied");
        assert.equal(gitText(projectRoot, ["rev-parse", "HEAD"]), beforeRecovery, "recovery cannot merge again");
        const journal = JSON.parse(readFileSync(journalPath, "utf8"));
        assert.equal(journal.nodes["T1.1"].handoff.nextAction, "Record existing integration");
      }
...
test("compiled MCP presents and normalizes a client-native workflow form", async () => {
  const client = new Client(
    { name: "codex_cli_rs", version: "1.0.0" },
...
test("proposal workflows validate staged plan files without transporting their content", () => {
  ...
  assert.match(track, /candidate_apply.*mode: "prepare"/);
  assert.match(review, /candidate_apply/);
...
test("implementation guidance preserves approval, permission, semantic checkpoints, and task-commit boundaries", () => {
  ...
  assert.match(implement, /commit group/);
...
test("semantic authorization envelopes remove deterministic follow-up approvals", () => {
  ...
  assert.match(track, /one combined commit/);
  assert.equal(trackState.operation, null);
  assert.equal(trackState.schemaVersion, 2);
