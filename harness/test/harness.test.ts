...
test("installer prepares a dual-product user plugin marketplace", async () => {
  ...
  assert.equal(codexManifest.version, "3.0.2+codex.test-build");
  assert.equal(claudeManifest.version, "3.0.2+claude.test-build");
  ...
  assert.equal(previousManifest.version, "3.0.2+codex.test-build");
  const updatedManifest = JSON.parse(readFileSync(join(target, "plugins", "cadre", ".codex-plugin", "plugin.json"), "utf8"));
  assert.equal(updatedManifest.version, "3.0.2+codex.second-build");
});
