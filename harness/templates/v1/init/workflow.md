...
Treat semantic content, scope, and consequences as proposals until the human reviews and explicitly approves them. One approval creates an authorization envelope over the complete exact proposal and all unchanged deterministic consequences named in it, including operation journals, generated indexes, validation, commits, lifecycle transitions, and provenance recording. Those mechanics do not become separate human decisions. Persist the authorization and checkpoint before pausing a multi-stage flow so another session can resume it.

Render unapproved artifact bodies into a visible project-local candidate stage under `<project-root>/.cadre-stage/<candidate-id>/`, never into canonical `.cadre/` and never into an MCP request. Candidate paths are constrained relative paths, stages reject symbolic links and oversized files, and `.cadre-stage/` is never included in a Git commit. Use `artifact_candidate_manifest` or the workflow-specific candidate preview to bind approval to exact path/SHA-256 entries. After approval, journal that digest and manifest, promote only matching staged bytes, verify their canonical hashes, and retain the stage through the artifact commit checkpoint for interruption recovery. Remove only that matching stage after commit provenance is recorded. A changed candidate requires a fresh preview and approval.

Present exact content or focused diffs, consequences, unknowns, and verification evidence once at the decision boundary. Combine related decisions whenever their complete artifacts and consequences are available together. In particular, do not split create configuration from initialization, a clear track specification from its validated plan, finding disposition from prepared remediation, project refresh from its exact cascading track changes, revert intent from deterministic reconciliation, reject-all risk acceptance from clean-review completion, or a uniquely determined archive selection from its full batch proposal. A response must unambiguously approve every included semantic consequence. Require a corrected proposal and new approval only when content, scope, selection, digest-bound semantics, verification exceptions, or newly discovered consequences materially change.

An `implement` invocation authorizes execution using its persisted `approvalMode`: `governed`, `phase`, or `autonomous`. This is scoped authorization for work already bounded by the approved specification and plan, not permission to guess through material ambiguity, expand scope, waive required checks, perform destructive or remote actions, or conceal conflicts and exceptions. `phase` is the default unless the human explicitly requests another approval mode; a resumed execution inherits its recorded mode. Starting a `phase` or `autonomous` execution from that invocation does not require a separate approval prompt.

### Human interaction forms

For a required clarification or human decision, first show only the concise context, focused diff, consequences, and evidence needed to decide. Inspect the active host policy before calling the read-only `workflow_elicit` MCP tool. If the task context reports approval policy `never`, including Codex Full Access, skip the form and ask the same short question once in chat. Otherwise prefer `workflow_elicit` so Codex and Claude can render the same client-native form. Use `clarification` with at most three flat questions. Use `approval` only after the exact proposal or verification evidence is ready, and bind it to the current preview digest or an immutable checkpoint containing the relevant track, execution/node, and commit. An `approved` result is explicit human approval of that binding; `changes_requested`, `declined`, and `cancelled` are not approval.

If the client does not advertise form elicitation or rejects the request with `fallback_required`, ask the same short question once in chat and do not retry the form. If `workflow_elicit` was mistakenly called while the task explicitly reports policy `never` and immediately returns `declined`, treat that response as policy rejection and use the same chat fallback; do not report it as a human decline. Never use form elicitation for passwords, API keys, access tokens, payment credentials, or other secrets. Do not print a complete unchanged workflow or other large default artifact merely to create context for a form; identify it by path/version/hash and summarize it unless the human asks for the full content.
...
### Create-time configuration envelope

During `create`, identify the default workflow by template ID, version, hash, and a concise summary. Include it with product context, Git disposition, styleguides, generated state, and initialization consequences in one final approval envelope. Do not create separate workflow, styleguide-selection, manifest, Git-initialization, validation, or commit approvals. Ask before the final proposal only when a material workflow or convention choice is unresolved; a clarification answer is input to the final proposal, not an additional approval stage.

Always propose `styleguides/general.md`. Match the proposed tech stack against the bundled default catalog, then propose the applicable language, framework, and build-tool guides as one set. Let the human request an amended default or a user-provided replacement without asking them to accept every guide separately. Framework/tool guides supplement their language guide: HTML/CSS underpins browser UI; TypeScript supplements JavaScript; React for the web supplements HTML/CSS plus JavaScript or TypeScript; Flutter supplements Dart; SwiftUI supplements Swift; Maven or Gradle supplements Java/Kotlin. In brownfield projects, existing enforced conventions take precedence unless the final envelope explicitly replaces them.

### Interruption-safe operations

At command entry, call `project_status` once and use its embedded structured validation. Reserve a separate `state_validate` call for final mutation gates or focused diagnostics; do not perform redundant full-project validation scans. Derived-state warnings such as a stale generated `tracks.md` are resumable drift, not canonical corruption. Report and repair them through the MCP render gate within the owning authorization envelope before a final commit.
...
Advance the checkpoint after each durable artifact write. Once an artifact commit is identified, append it to history and advance the lifecycle checkpoint. Preserve or transition the same operation journal while later stages remain inside one authorization envelope, as with combined track specification and planning; clear it only after every authorized stage and provenance record is complete. This protocol makes interruption before a commit, between commits, after a commit, or before follow-up bookkeeping resumable.

During `create`, detect an existing worktree with `git rev-parse --show-toplevel` and never initialize a nested repository. If no worktree exists, record the approved project root and `initialize` disposition through the adaptive MCP initialization command before running `git init` there. Its first call returns `approval_required`; call it with the proposal token only after the human approves. Verify the resulting root and record that checkpoint before the setup commit. Resume a pending initialization from the journal; stop if the observed repository conflicts with the recorded disposition or root.

For deterministic MCP mutations already covered by the active authorization envelope, call the adaptive command once; it computes the proposal, validates current state, and applies atomically. A command returns `approval_required` plus an opaque proposal token only when a new human decision is required. Present that exact proposal, wait for approval, then call the same command with only its token. Never make proposal and token calls consecutively without human input. Do not inspect the global tool catalog or probe commands to rediscover declared contracts. In `phase` or `autonomous` implementation, execution start plus deterministic worktree, clean integration, cleanup, journal, index, and bookkeeping mutations already covered by the invocation run in one call and do not create new approval prompts. A stale token is refreshed before presentation rather than forcing duplicate approval. The MCP never grants approval or replaces required repository inspection. Its Git surface is limited to constrained Cadre worktree creation, integration, and safe cleanup; it accepts no arbitrary command, path, force deletion, or automatic conflict resolution.

Resuming a recorded authorization envelope never requires another human approval when the operation journal has the same approval/content digest and the newly previewed artifacts are identical to the approved proposal, with only checkpoint progress changed. Any content, selection, semantic consequence, or approval-digest mismatch requires a corrected proposal and new approval.

When a command skill explicitly instructs candidate staging or direct promotion for Cadre artifacts and no corresponding MCP mutation tool is declared, those filesystem writes are intentional. Do not search the global tool list for an undeclared operation. Manifest candidates before approval, journal the approved digest first, use the available MCP validation and derived-render gates, and stop if the promoted state does not match the approved hashes.
...
- `phase` (default): execute regular work autonomously but pause at each phase's final `User Manual Verification` task.
...
Git ancestry is derived from the current canonical or registered phase-worktree HEAD. Task branches merge without squashing into the registered phase worktree when one exists, otherwise into canonical main for explicitly main-coordinated work. Phase branches merge without squashing into canonical main. Cadre tool names are direct contracts: semantic execution events use one atomic `execution_checkpoint` call. Worktree tools derive phase and base from the node ID and current Git state. `integration` is atomic in `phase` or `autonomous` mode and returns `approval_required` before mutation in `governed` mode.
...
3. Derive ready work from the journal and allocate workers by critical path, downstream work unblocked, then plan order. Send semantic checkpoint events only after their commit, verification, integration, or approval evidence exists; the server expands each event into its complete legal transition sequence.
...
6. Run focused checks per task, combined phase checks after task integration, and the relevant broader suite after conflict resolution, phase integration, and before track verification. Record command, working directory, result, and verified product HEAD/tree; reuse a passing result while product files, test inputs, and policy are unchanged. `.cadre/**`-only bookkeeping does not invalidate product verification.
...
9. Phase-level manual verification becomes ready only after every sibling task is integrated. Run its technical evidence in the phase worktree through the phase worker when present, otherwise through main. `governed` and `phase` require one human approval at this final phase task; `autonomous` records the verified evidence without a prompt. Afterward checkpoint, phase integration, provenance, learning, and cleanup are deterministic consequences and do not create extra approvals outside `governed`. Keep the phase node `integrated` through safe worktree cleanup, then mark it `completed` and start dependents. Cleanup may recover an already-`completed` node without reopening that terminal state. A phase remains `running` until all sibling tasks and this barrier are `completed`; integration and cleanup must not run early.
...
Product task commits remain distinct provenance boundaries. Cadre-only learning and journal bookkeeping is committed at meaningful checkpoints, not per internal status. Execution finish derives HEAD/time and atomically applies plan evidence, the completed journal, `ready_for_review` state, and `tracks.md`, so do not run marker or index repair workflows.
...
A semantic revision requires an approved specification baseline and no unresolved non-revision operation. Reconcile a matching revision journal before doing new work. If active implementation or dirty work overlaps the revision, include its exact complete, preserve-as-superseded, or additive-revert disposition in the combined revision envelope whenever it can be safely assessed in advance. Ask for a preliminary disposition only when that choice is required to reach a state that can be fully analyzed.
