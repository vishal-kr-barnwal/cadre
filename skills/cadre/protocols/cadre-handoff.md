...
Use the workflow arguments to choose the handoff mode.

## 0. Mode

If workflow arguments contain `--for-teammate`, run the **Teammate Handoff** flow in
...
- `<git-identity>` is the value of `git config user.email` (fallback
  `git config user.name`, else null). Compute it once at the start of this workflow.
...
- Resume workflow (`cadre-implement <track_id>`)
- Next action to take
...
**Run only when workflow arguments contain `--for-teammate`.** Produces a goal-first,
...
2. **Compute identities & timestamp:** `<git-identity>` (the handoff author) =
   ...
   `--for-teammate` in workflow arguments (e.g. `--for-teammate alice@example.com` or
   ...
   *describes* the handoff — this step makes the track actually land in the
   ...
   - **Set the ASSIGNEE + `handoff:pending` label — never the owner.** Ownership
     ...
     bd update <epic_id> --assignee "<recipient-identity>" \
       --add-label handoff:pending --json
     ```
     (If `--add-label` is not combinable with `--assignee` in one call, run
