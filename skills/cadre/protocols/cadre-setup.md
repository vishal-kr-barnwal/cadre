...
> The generated protocol still requires `cadre_ping` before setup starts.
...
2. **Execute Based on Maturity:**
   ...
   **IF GREENFIELD:**
   ...
   - Upon response:
     ...
     - Call `cadre_current_root` with `root` (the current project root or any path
...
2. **Persist the choice** in `cadre/setup_state.json` (merge, don't overwrite
   ...
   - Call `cadre_current_root` with `root` if this is the first point where
...
5. **MCP polyrepo sanity check:** Call `cadre_polyrepo_preflight` with `root`.
...
5. **Seed the Tracks Index:** Create `cadre/tracks.md` as a **derived index**
   ...
   This list is regenerated by MCP `cadre_regen_index`; edit `metadata.json`, not the markers below.
   ...
   After writing the seed file, call `cadre_regen_index` with `root` to verify the
