...
5. **Seed the Tracks Index:** Create `conductor/tracks.md` as a **derived index**
   ...
   This list is regenerated by `/conductor-status --regen-index`; edit `metadata.json`, not the markers below.

   <!-- conductor:index:start -->
   <!-- conductor:index:end -->
   ```
   The first track created (below / via `/conductor-newtrack`) is spliced into
